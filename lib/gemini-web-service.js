...
/**
 * Profile dịch tạm, tách hoàn toàn khỏi profile đăng nhập. Gemini hiện cho phép
 * dùng ô nhập ở chế độ khách; tách profile tránh khóa/corrupt phiên đăng nhập.
 */
function getGeminiTranslationProfileDir() {
  const customDir = process.env.GEMINI_TRANSLATION_PROFILE_DIR;
  const profileDir = customDir && String(customDir).trim()
    ? String(customDir).trim()
    : path.join(os.tmpdir(), 'video-studio-gemini', `vs_gemini_profile_${process.pid}`);
...
async function captureGeminiFailure(page, reason, prompt = '', logFn = console.log) {
...
- DỊCH 1:1 (RÀNG BUỘC CỨNG, KHÔNG ĐƯỢC PHÁ dù viết lại tự do về CÁCH NÓI): MỖI dòng gốc VẪN → ĐÚNG 1 dòng dịch, đúng thứ tự, TUYỆT ĐỐI KHÔNG gộp/tách/bỏ dòng — mỗi dòng gốc gắn 1 khe thời gian cố định trong video, gộp/tách sẽ làm LỆCH đồng bộ hình-tiếng (tool dedupe câu trùng TRƯỚC khi gửi — bạn KHÔNG cần lo lặp).
  ...
  ⚠ ĐOẠN CUỐI/LỜI BÌNH có NHIỀU DÒNG NGẮN LIÊN TIẾP (3-6 chữ, cùng 1 mạch ý — vd lời bình kết phim) là nơi DỄ PHẠM LUẬT 1:1 NHẤT: bạn sẽ có xu hướng gộp chúng thành 1 câu văn xuôi trôi chảy cho tự nhiên — TUYỆT ĐỐI KHÔNG LÀM VẬY. VD 3 dòng gốc '却又在最高潮处' / '无情撕碎了人定胜天的幻想' / '电影安东败给闯红灯的路人' PHẢI ra ĐÚNG 3 dòng dịch riêng (mỗi dòng ý ngắn/cụt cũng được, KHÔNG được nối chúng thành 1 câu dài rồi bỏ 2 dòng kia). Nghe rời rạc hơn văn xuôi KHÔNG SAO — đây là PHỤ ĐỀ đồng bộ theo khung hình, không phải văn bản đọc liền mạch.\n`;
    if (fit) {
      prefix += `- KHỚP LỒNG TIẾNG (QUAN TRỌNG — đọc kỹ): đầu mỗi dòng có [Ts ≤N từ] = câu này có T GIÂY để TTS đọc, và ĐỌC KỊP thì bản dịch KHÔNG ĐƯỢC QUÁ N TỪ. Đếm rất dễ: mỗi từ cách nhau 1 dấu cách ('Anh ấy tới rồi' = 4 từ). ĐÂY LÀ TRẦN CỨNG — vượt N từ là giọng đọc bị ép nhanh, méo tiếng.
  ...
  VÍ DỤ (Gốc → SAI dài → ĐÚNG ngắn), để ý số TỪ: '他来了' → 'Anh ấy đã đến nơi đây rồi đấy' (8 từ, SAI) → 'Anh ấy tới rồi' (4 từ, ĐÚNG); '太美了' → 'Cái này thực sự là quá đẹp đi mà' (8 từ, SAI) → 'Đẹp quá' (2 từ, ĐÚNG); '我不知道该怎么办' → 'Tôi thực sự không biết mình nên phải làm thế nào' (11 từ, SAI) → 'Không biết làm sao' (4 từ, ĐÚNG).\n`;
    ...
    prefix += `LƯU Ý INPUT: '@HH:MM:SS' đầu mỗi dòng = MỐC GIỜ câu xuất hiện trong video — CHỈ để bạn hiểu TIMING + nhận ra câu TRÙNG kề nhau (2 dòng sát giờ + cùng ý = phụ đề song ngữ). TUYỆT ĐỐI KHÔNG in '@HH:MM:SS' vào bản dịch.\n`;
    if (fit) {
    ...
    prefix += `You are a NATIVE ${targetName} SPEAKER retelling the story below in ${targetName} - you are NOT a word-for-word translation machine. Write in the native script of ${targetName} - do NOT romanize (e.g. Hangul for Korean, Thai script for Thai).
...
- 1:1 TRANSLATION (HARD CONSTRAINT, DO NOT BREAK even though you rewrite freely in WORDING): EACH source line MUST STILL map to EXACTLY 1 translated line, same order - NEVER merge/split/drop lines (each source line is tied to a fixed time slot in the video; merging/splitting breaks audio-video sync). OUTPUT LINE COUNT MUST EQUAL INPUT LINE COUNT.
  ...
  ⚠ A RUN OF MANY SHORT CONSECUTIVE LINES (3-6 words, same train of thought - e.g. closing commentary) is where you are MOST LIKELY to break the 1:1 rule: you will be tempted to merge them into one flowing sentence for naturalness - DO NOT. Each of those short lines still gets its OWN translated line, even if it reads choppy/fragmented — this is a TIME-SYNCED SUBTITLE, not prose meant to be read as one paragraph.\n`;
    if (fit) {
      ...
      prefix += `- DUB TIMING (CRITICAL): each line starts with [Ts <=N] = this line has T SECONDS for the TTS voice. Keep it SHORT to fit — [<=N] is a length ANCHOR (don't literally count characters, just write it tight). 'Keep meaning' HERE = keep the CORE INFO (who does what, the facts/numbers) and you ARE ALLOWED to DROP: filler words, connectors, and pronouns/subjects when context is clear. Do NOT write long out of caution — trimming filler does NOT lose plot. NEVER print [Ts <=N] in the translation.\n`;
    ...
    prefix += `INPUT NOTE: '@HH:MM:SS' at the start of each line = the TIME MARK the line appears in the video - ONLY for you to understand TIMING + spot ADJACENT DUPLICATE lines. NEVER print '@HH:MM:SS' in the translation.\n`;
    if (fit) {
      prefix += `★ REMINDER (most important): every line must be SHORT to fit its [Ts <=N] slot - keep CORE INFO, cut fillers/connectors/optional pronouns, use the language's short forms. A long-winded line = ERROR.\n`;
    }
    prefix += `RETURN ONLY THE TRANSLATION in ${targetName}: one line per sentence, KEEP the exact numbering 1. 2. at the start of each line, same order, do NOT merge or split lines. NEVER add anything else - no preamble, no confirmation, no explanation, no markdown:\n\n`;
  }

  const formattedLines = items.map((it, idx) => {
    const duration = slotGiay(it.timestamp);
    const wps = Number(process.env.DUB_WPS || 5.0) || 5.0;
    const ratio = Number(process.env.DUB_TILE_AM || 0.75) || 0.75;
    const cps = Number(process.env.DUB_CPS || 16) || 16;
    const limit = fit && duration > 0
      ? (isVietnamese ? tranTuDich(duration, it.text, wps, ratio) : Math.max(10, Math.round(duration * cps)))
      : 0;
    const anchor = limit > 0
      ? `[${duration.toFixed(1)}s ≤${limit}${isVietnamese ? ' từ' : ''}] `
      : '';
...
async function getGeminiSession(show = false, logFn = console.log) {
  ...
  logFn(`🔥 Gemini Web mở phiên khách tạm riêng với Profile: ${profileDir}`);
...
async function closeGeminiSession() {
  const profileDir = _GEM_PROFILE_DIR;
  try {
    if (_GEM_CTX) {
    ...
    _GEM_FIRST_EDITOR_WAIT = true;
    if (profileDir && !process.env.GEMINI_TRANSLATION_PROFILE_DIR) {
...
function nghiGopCau(translatedMap, minimumMedian = 8, multiplier = 3.0, absoluteFloor = 60) {
  ...
  return median >= minimumMedian && lengths[lengths.length - 1] > Math.max(absoluteFloor, median * multiplier);
}
...
async function translateSrtItemsByGeminiWeb(items, options = {}) {
  ...
  const retryRounds = Math.max(0, Math.min(5, Number(options.retryRounds ?? process.env.GEM_RETRY_SOT ?? 3) || 0));
  ...
  const requestBatch = async (batch, attempts, label) => {
    ...
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
      ...
      const validate = responseText => {
        ...
        if (nghiGopCau(parsed)) return false;
        const validCount = batch.reduce((count, item, index) => (
          count + (isGeminiTranslationValid(item.text, parsed[index + 1], {
            targetLang,
            hanThreshold: Number(process.env.HAN_NET_NGUONG || 0.35),
            mode
          }).valid ? 1 : 0)
        ), 0);
        return validCount >= Math.ceil(batch.length * 0.9);
      };
      try {
        logFn(`[Gemini Web] ${label} · lần ${attempt}/${attempts} · ${batch.length} cue.`);
        const response = await requestFn(prompt, { logFn, validate, show: options.show === true });
        const parsed = parseResponseLo(response, batch.length);
        const score = batch.reduce((count, item, index) => (
          count + (isGeminiTranslationValid(item.text, parsed[index + 1], {
            targetLang,
            hanThreshold: Number(process.env.HAN_NET_NGUONG || 0.35),
            mode
        ...
        if (validate(response)) return parsed;
        logFn(`⚠ ${label} mới đạt ${score}/${batch.length} cue hợp lệ; Gemini sẽ dịch lại lô này.`);
      } catch (error) {
  ...
  let initial = pendingItems();
  const initialBatchCount = Math.ceil(initial.length / batchSize);
  for (let offset = 0, batchIndex = 0; offset < initial.length; offset += batchSize, batchIndex += 1) {
    const batch = initial.slice(offset, offset + batchSize);
    const attempts = initialBatchCount > 1 ? 3 : 2;
    const parsed = await requestBatch(batch, attempts, `lô ${batchIndex + 1}/${initialBatchCount}`);
    const accepted = acceptMap(batch, parsed);
    logFn(`[Gemini Web] Đã ghi lũy tiến lô ${batchIndex + 1}/${initialBatchCount}: ${accepted}/${batch.length} cue mới.`);
    if (batchIndex + 1 < initialBatchCount) await wait(batchDelayMs);
  }
  ...
  for (let round = 1; round <= retryRounds; round += 1) {
    ...
    let progress = 0;
    const rescueBatchCount = Math.ceil(missing.length / batchSize);
    logFn(`🔁 Gemini Web còn ${missing.length} cue sót → dịch lại phần sót lần ${round}/${retryRounds}.`);
    for (let offset = 0, batchIndex = 0; offset < missing.length; offset += batchSize, batchIndex += 1) {
      const batch = missing.slice(offset, offset + batchSize);
      const parsed = await requestBatch(batch, 2, `vá ${round} · lô ${batchIndex + 1}/${rescueBatchCount}`);
      progress += acceptMap(batch, parsed);
      if (batchIndex + 1 < rescueBatchCount) await wait(batchDelayMs);
    }
    if (progress === 0) {
      logFn(`⚠ Retry Gemini Web không dịch thêm được cue nào; còn ${missing.length} cue sót.`);
      break;
    }
  }
