...
function buildRetryCorrection(assessment, expectedCount) {
  ...
  const allIds = `1 đến ${expectedCount}`;
  if (assessment.reasonCode === 'merged_cue') {
    const ids = (assessment.mergedKeys || []).join(', ') || 'không xác định';
    return `\n\n⚠ SỬA LỖI LƯỢT TRƯỚC: Bạn đã DỒN NHIỀU cue vào một câu quá dài (ID: ${ids}). Lần này MỖI SỐ chỉ được chứa bản dịch của CHÍNH dòng mang số đó. Trả đủ từng số ${allIds}; không gộp, không tách, không chuyển nội dung sang số khác.`;
  }
  ...
  if (assessment.reasonCode === 'missing_cluster' || assessment.reasonCode === 'insufficient_coverage') {
    return `\n\n⚠ SỬA LỖI LƯỢT TRƯỚC: Kết quả bị thiếu cue (${assessment.reason}). Hãy trả ĐỦ chính xác từng số ${allIds}, kể cả dòng rất ngắn hoặc khó hiểu; không bỏ, gộp hay đánh lại số.`;
  }
  return `\n\n⚠ SỬA LỖI LƯỢT TRƯỚC: ${assessment.reason}. Hãy trả đủ từng số ${allIds} và tuân thủ đúng định dạng.`;
}
...
async function translateSrtItemsByGeminiWeb(items, options = {}) {
  ...
  const acceptMap = async (batch, parsed) => {
    ...
    for (let index = 0; index < batch.length; index += 1) {
      ...
      const value = String(safe.cleaned[index + 1] || '').trim();
  ...
  const requestBatch = async (batch, attempts, label) => {
    ...
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
      ...
      const prompt = buildPrompt(batch, tmContent, targetLang, options.fit !== false, mode)
        + (attempt > 1 ? buildRetryCorrection(previousAssessment, batch.length) : '');
