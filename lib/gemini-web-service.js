 ...
 */
function normalizeSubtitleSourceKind(value) {
  const source = String(value || '').trim().toLowerCase();
  if (['whisper', 'faster-whisper', 'capcut-asr', 'capcut', 'asr', 'speech'].includes(source)) {
    return 'asr';
  }
  if (source === 'hybrid' || source.includes('ocr+whisper') || source.includes('ocr+capcut')) {
    return 'hybrid';
  }
  if (['upload', 'saved', 'user-subtitle', 'manual'].includes(source)) {
    return 'user-subtitle';
  }
  return 'ocr';
}

function getSourceRecoveryRule(sourceKind) {
  const normalized = normalizeSubtitleSourceKind(sourceKind);
  const ocrRule = `1. XỬ LÝ LỖI OCR BẢN GỐC: dữ liệu quét từ khung hình có thể sai mặt chữ, sinh từ vô nghĩa hoặc phá vỡ ngữ pháp. Nếu một từ không hợp câu, phải dựa vào các dòng trước/sau để đoán chữ đúng và dịch theo ý đúng; không bám máy móc vào chữ OCR sai.`;
  const asrRule = `1b. NGUỒN NÀY LÀ NHẬN DẠNG GIỌNG NÓI (Whisper/CapCut ASR), nên lỗi thường là NGHE NHẦM ĐỒNG ÂM: từ nhận được có thể là từ có thật và đúng ngữ pháp nhưng sai hoàn toàn mạch truyện. Với mỗi từ đáng ngờ, phải kiểm tra nó có hợp sự việc ở các dòng trước/sau không; nếu lạc ý thì suy ra từ cùng âm hợp ngữ cảnh rồi dịch nghĩa đúng. Ví dụ lỗi thật: 力气 (sức lực) bị nghe thành 利器 (vũ khí), 要房 (đòi nhà) thành 药房 (hiệu thuốc), 贫道 (bần đạo) thành 频道 (kênh). Chỉ sửa từ nghe nhầm, không bịa thêm tình tiết.`;
  if (normalized === 'asr') return asrRule;
  if (normalized === 'hybrid') return `${ocrRule}\n${asrRule}`;
  if (normalized === 'user-subtitle') {
    return '1. NGUỒN LÀ PHỤ ĐỀ DO NGƯỜI DÙNG CUNG CẤP: tôn trọng nguyên văn. Chỉ suy sửa lỗi rõ ràng khi ngữ cảnh chứng minh chắc chắn; không tự coi mọi từ lạ là lỗi OCR hoặc lỗi nghe.';
  }
  return ocrRule;
}

function getQuyTacChung(sourceKind = 'ocr') {
  const sourceRecoveryRule = getSourceRecoveryRule(sourceKind);
...
2b. KHÔNG ĐỂ LỌT CHỮ HÁN — NHƯNG CŨNG KHÔNG PHIÊN ÂM: bản dịch không được còn BẤT KỲ ký tự chữ Hán nào (kể cả 1 chữ lẻ dính trong từ, vd 技, 剣) hay pinyin. Từ gốc Hán phải được dịch sang NGHĨA tiếng Việt (vd 剣技 → 'kiếm thuật' / 'kỹ năng').
   ...
   ✅ THƯƠNG HIỆU, SẢN PHẨM hoặc CÔNG TY hiện đại có tên Latin chính thức phải giữ dạng Latin phổ biến: 恰恰 → ChaCha; 华为 → Huawei; 小米 → Xiaomi; 抖音 → Douyin; 比亚迪 → BYD. Không phiên âm Hán-Việt thương hiệu đã có tên Latin quen thuộc.
...
4. CON SỐ VÀ ĐƠN VỊ: giữ đúng mọi con số, phần trăm, đơn vị đo và số tiền của CHÍNH dòng nguồn; không mượn số ở dòng trước/sau, không tự làm tròn hoặc đổi bậc. Quy đổi đúng: 万 = 10 nghìn; 十万 = 100 nghìn; 百万 = 1 triệu; 千万 = 10 triệu; 亿 = 100 triệu. Ví dụ 那整1万多呢 phải là “hơn 10 nghìn”, không phải “10 triệu”; 我就要那10% phải giữ đúng 10%.
5. TUYỆT ĐỐI KHÔNG ĐỂ SÓT KÝ TỰ CHỮ HÁN trong bản dịch tiếng Việt, kể cả một chữ nằm giữa câu như “Liễu di娘”, “mắt của奶奶”, “đi抄家”. Phải dịch hoặc chuyển tên hoàn chỉnh; không chèn ghi chú, mũi tên hay bình luận của người dịch vào câu thoại.
=== HẾT QUY TẮC ===\n\n`;
}
...
function buildPrompt(items, tmContent, targetLang = 'vi', fit = true, mode = 'translate', options = {}) {
  ...
  let prefix = '';
  if (mode === 'spellcheck') {
    const spellcheckSource = sourceKind === 'asr'
      ? 'phần mềm nhận dạng giọng nói (Whisper/CapCut ASR), có thể sai dấu thanh, từ đồng âm và dấu câu'
      : (sourceKind === 'hybrid'
          ? 'OCR kết hợp nhận dạng giọng nói, có thể sai mặt chữ, từ đồng âm và dấu câu'
          : (sourceKind === 'user-subtitle'
              ? 'người dùng cung cấp; chỉ sửa lỗi chính tả và dấu câu rõ ràng'
              : 'OCR từ khung hình, có thể sai mặt chữ và dấu câu'));
    prefix = `Dưới đây là phụ đề do ${spellcheckSource}. Nhiệm vụ của bạn: KIỂM TRA và SỬA LỖI CHÍNH TẢ + dấu câu cho từng dòng.
...
CHỈ trả về kết quả: mỗi dòng MỘT câu, CHÉP NGUYÊN mã [ID_xxx] tương ứng ở đầu dòng, KHÔNG tự đánh lại mã, không gộp/tách dòng. TUYỆT ĐỐI KHÔNG thêm gì khác — không lời dẫn, không giải thích, không markdown:\n\n`;
    ...
    prefix = getQuyTacChung(sourceKind);
    if (tmContent) {
...
- CHẤM CÂU THEO MẠCH nhưng vẫn giữ 1:1: dòng còn tiếp ý sang dòng sau phải kết bằng dấu phẩy hoặc không có dấu cuối; chỉ dùng dấu chấm, hỏi, cảm khi ý thật sự kết thúc. Không được gắn dấu chấm máy móc cho mọi cue vì sẽ phá nhịp lồng tiếng.
- KHẨU HIỆU, THÀNH NGỮ, CHƠI CHỮ và CỤM PHỦ ĐỊNH phải dịch theo nghĩa cả cụm. Cấu trúc như 不满意不要钱 / 不…不收费 / 不…包退 phải hiểu là “không đạt điều kiện thì miễn phí/không thu tiền/hoàn trả”, không được lật thành “không miễn phí”. Nguồn ASR/OCR thường không có dấu; hãy tách cụm theo ngữ pháp trước khi dịch.\n`;
    if (fit) {
  ...
  VÍ DỤ (Gốc → SAI dài → ĐÚNG ngắn), để ý số TỪ: '他来了' → 'Anh ấy đã đến nơi đây rồi đấy' (8 từ, SAI) → 'Anh ấy tới rồi' (4 từ, ĐÚNG); '太美了' → 'Cái này thực sự là quá đẹp đi mà' (8 từ, SAI) → 'Đẹp quá' (2 từ, ĐÚNG); '我不知道该怎么办' → 'Tôi thực sự không biết mình nên phải làm thế nào' (11 từ, SAI) → 'Không biết làm sao' (4 từ, ĐÚNG).\n`;
    ...
    const sourceErrorRule = sourceKind === 'asr' || sourceKind === 'hybrid'
      ? 'SOURCE IS SPEECH RECOGNITION OUTPUT: errors are often valid homophones that remain grammatical but contradict the story. Check suspicious words against adjacent lines and infer the contextually correct homophone without inventing facts.'
      : (sourceKind === 'user-subtitle'
          ? 'SOURCE IS USER-PROVIDED SUBTITLE TEXT: preserve it faithfully and correct only errors clearly proven by context.'
          : 'SOURCE HAS OCR ERRORS: the input was scanned off video frames, so some characters are misread and produce nonsense words or break the grammar. When a word breaks the sentence, infer the intended word from surrounding lines.');
