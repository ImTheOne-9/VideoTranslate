...
const DEPENDENCIES = {
  ...
  separator: {
    name: 'MDX ONNX Audio Separator',
    url: 'https://huggingface.co/datasets/dvh1910/video-studio-tools/resolve/main/mdx-onnx-separator.zip',
    zipName: 'mdx-onnx-separator.zip',
    expectedFiles: [
...
const LEGACY_SEPARATOR_FILES = [
  'audio-separator.exe',
  'audio-separator.zip',
  'audio-separator-gpu.zip',
  'setup_gpu_separator.ps1',
  'separate_audio.py',
  'check_cuda.py',
  path.join('models', 'bs_roformer_ep317_sdr12.9755.onnx'),
  path.join('models', 'bs_roformer_ep317_sdr12.9755.onnx.data'),
  path.join('models', 'bs_roformer_ep317_sdr12.9755_quantized_uint8.onnx'),
  path.join('models', 'model_bs_roformer_ep_317_sdr_12.9755.ckpt'),
  path.join('models', 'model_bs_roformer_ep_317_sdr_12.9755.yaml')
];

function cleanupLegacySeparator(toolsDir) {
  for (const relativePath of LEGACY_SEPARATOR_FILES) {
    try {
      fs.rmSync(path.join(toolsDir, relativePath), { force: true });
    } catch (error) {
      console.warn(`[Dependency Downloader] Không thể xóa tài nguyên separator cũ ${relativePath}: ${error.message}`);
    }
  }

  const legacyPythonEnv = path.resolve(toolsDir, '..', 'temp_env');
  try {
    fs.rmSync(legacyPythonEnv, { recursive: true, force: true });
  } catch (error) {
    console.warn(`[Dependency Downloader] Không thể xóa Python env cũ: ${error.message}`);
  }
}
...
module.exports = { checkDependencyStatus, cleanupLegacySeparator, DEPENDENCIES, downloadAndExtract };
