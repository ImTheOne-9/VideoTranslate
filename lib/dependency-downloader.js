...
const DEPENDENCIES = {
  ...
  'separator-gpu': {
    name: 'Audio Separator (GPU)',
    url: 'https://huggingface.co/datasets/dvh1910/video-studio-tools/resolve/main/audio-separator-gpu.zip',
    zipName: 'audio-separator-gpu.zip',
    expectedFiles: ['setup_gpu_separator.ps1', 'separate_audio.py']
  }
};

// Hàm kiểm tra xem đã đầy đủ thư viện hay chưa
function checkDependencyStatus(toolsDir) {
  const status = { cuda: true, whisper: true, separator: true, separatorGpu: false };
  
  for (const [key, dep] of Object.entries(DEPENDENCIES)) {
    if (key === 'separator-gpu') continue;
  ...
  // GPU separator: kiểm tra venv ở thư mục gốc project
  const appRoot = path.resolve(toolsDir, '..');
  const venvPython = path.join(appRoot, 'temp_env', 'Scripts', 'python.exe');
  status.separatorGpu = fs.existsSync(venvPython);
