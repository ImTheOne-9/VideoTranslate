...
function validateForType(media, type) {
  ...
  const limit = (name, fallback) => {
    ...
    if (!Number.isFinite(value) || value <= 0) throw new Error(`Cấu hình ${name} phải là số lớn hơn 0`);
    return value;
  };
  ...
  if (type === 'story' || type === 'reel') {
    const maxSeconds = type === 'story' ? limit('FACEBOOK_STORY_MAX_SECONDS', 60) : limit('FACEBOOK_REEL_MAX_SECONDS', 90);
    if (media.duration > maxSeconds) throw new Error(`${type === 'story' ? 'Story' : 'Reel'} dài ${media.duration.toFixed(1)} giây, vượt ngưỡng ${maxSeconds} giây đang cấu hình trong phần mềm. Đây chưa phải lỗi do Facebook trả về. Với video dài, hãy chọn Video Post hoặc điều chỉnh cấu hình sau khi xác minh API.`);
  }
  return media;
}
