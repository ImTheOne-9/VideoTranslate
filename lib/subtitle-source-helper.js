...
async function resolveAutomaticSubtitle(options, dependencies = {}) {
  ...
  if (useViralPipeline) {
    ...
    let trackedResult;
    try {
      trackedResult = await runTrackedOcr(initialDevice);
    } catch (error) {
    ...
    if (!accepted) {
      if (ocrOnly) {
        throw new OcrNoSubtitlesError(trackedCueCount > 0
          ? `OCR ViralCrawl chỉ đọc được ${trackedCueCount} câu, chưa đạt tối thiểu 3 câu`
          : 'OCR ViralCrawl không tìm thấy phụ đề trong video');
      }
