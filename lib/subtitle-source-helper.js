...
const subtitleSourceCache = require('./subtitle-source-cache');
...
function findSuspiciousOcrGaps(cues, durationMs) {
  ...
  // Match ViralCrawl's opt-in hybrid behavior: short subtitles missed by OCR
  // leave gaps around 0.3-0.5s, so a 20s floor made the feature ineffective.
  const internalThreshold = 300;
  const gaps = [];
  // Never fill the leading edge: ASR commonly hallucinates credits/channel
  // names over an intro where the creator intentionally has no hard subtitle.
  ...
  if (sorted.length >= 6 && duration - lastEnd >= 30_000) {
    gaps.push({ startMs: lastEnd, endMs: duration, kind: 'tail' });
  }
  return gaps;
}
...
function mergeOcrAndWhisperCues(ocrCues, whisperCues, gaps) {
  ...
  const additions = (Array.isArray(whisperCues) ? whisperCues : []).filter((cue) => {
    ...
    if (!insideMissingRange || primary.some((ocrCue) => overlapsCue(cue, ocrCue))) return false;
    const normalized = meaningfulText(cue.text).toLowerCase();
    if (!normalized) return false;
    return !primary.some((ocrCue) => {
      const distance = Math.min(
        Math.abs(cue.startMs - ocrCue.endMs),
        Math.abs(ocrCue.startMs - cue.endMs)
      );
      if (distance > 1_000) return false;
      const other = meaningfulText(ocrCue.text).toLowerCase();
      return other && (other.includes(normalized) || normalized.includes(other));
    });
  });
  return {
...
async function resolveAutomaticSubtitleUncached(options, dependencies = {}) {
  const {
    videoPath,
    workDir,
    ffmpegPath,
  ...
  const writeNoSpeechResult = async (reason) => {
    const metadataPath = path.join(workDir, 'capcut-no-speech.json');
    ...
    return {
      // An empty recognition is valid, but an empty SRT is not valid input for
      // timeline normalization or translation. Keep this as explicit state.
      path: null,
      metadataPath,
      source: 'capcut-asr',
      language: resolvedSourceLanguage,
...
async function resolveAutomaticSubtitle(options, dependencies = {}) {
  if (options?.sourceCacheEnabled === false) {
    return resolveAutomaticSubtitleUncached(options, dependencies);
  }
  const createKey = dependencies.createSourceCacheKey || subtitleSourceCache.createSourceCacheKey;
  const restoreCache = dependencies.restoreSourceCache || subtitleSourceCache.restoreSourceCache;
  const saveCache = dependencies.saveSourceCache || subtitleSourceCache.saveSourceCache;
  const withCacheLock = dependencies.withSourceCacheLock || subtitleSourceCache.withSourceCacheLock;
  let key = null;
  try {
    key = await createKey(options);
    if (key) {
      const restored = await restoreCache(key, options.workDir);
      if (restored) {
        options.onProgress?.({ phase: 'subtitle_source_cache_hit', detail: { source: restored.source } });
        return restored;
      }
    }
  } catch {
  ...
  if (!key) return resolveAutomaticSubtitleUncached(options, dependencies);

  const outcome = await withCacheLock(key, async () => {
    const existing = await restoreCache(key, options.workDir);
    if (existing) return { cached: true };
    const generated = await resolveAutomaticSubtitleUncached(options, dependencies);
    const saved = await saveCache(key, generated, options.workDir);
    return { cached: false, generated, saved };
  }, {
    readAvailable: async () => (await restoreCache(key, options.workDir)) ? { cached: true } : null,
    onWait: () => options.onProgress?.({ phase: 'subtitle_source_cache_waiting' })
  });
  if (outcome?.saved || outcome?.cached) {
    const restored = await restoreCache(key, options.workDir);
    if (restored) return restored;
  }
  return outcome?.generated || resolveAutomaticSubtitleUncached(options, dependencies);
}
...
module.exports = {
  ...
  resolveAutomaticSubtitleUncached
};
