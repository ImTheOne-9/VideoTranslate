...
// Version 1 groups adjacent subtitle cues into natural speech segments. The
// original SRT remains unchanged; grouping applies only to narration synthesis.
...
class SegmentService {
  ...
  createOrLoad(options) {
    ...
    const segmentMode = cuePerSegment === true ? 'per-cue' : 'grouped';
    ...
    if (
      ...
      && (existing.segmentMode || 'grouped') === segmentMode
    ) {
      existing.reviewRequired = reviewRequired === true;
      const migratedToCue = existing.smartFit?.mode !== 'cue';
      existing.smartFit = { mode: 'cue' };
      if (migratedToCue) {
        for (const segment of existing.segments) {
          if (segment.audioFile) {
            const fittedPath = path.join(workDir, segment.audioFile);
            try { this.fs.rmSync(fittedPath, { force: true }); } catch {}
          }
    ...
    const groups = cuePerSegment === true
      ? finalCues.map((cue) => [cue])
      : groupCues(finalCues);
  ...
  setSegmentAudio(workDir, segmentId, audio) {
    ...
    if (Object.hasOwn(audio, 'narrationText')) {
      segment.text = normalizeText(audio.narrationText);
    }
    if (Object.hasOwn(audio, 'narrationFit')) {
      segment.narrationFit = audio.narrationFit || null;
    }
