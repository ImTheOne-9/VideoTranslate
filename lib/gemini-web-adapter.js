...
async function translateViaGeminiWeb(items, options = {}) {
  console.log('[GeminiWebAdapter] Dịch SRT bằng pipeline Gemini Web...');
  return translateSrtItemsByGeminiWeb(items, {
    ...options,
    logFn: options.logFn || ((msg) => {
      console.log(`[GeminiWeb] ${msg}`);
      options.onProgress?.(msg);
    })
  });
}

async function closeGeminiWebSession() {
  return closeGeminiSession();
}

module.exports = {
  getGeminiProfileDir,
  openGeminiLogin,
  closeGeminiWebSession,
  requestViaGeminiWeb,
