...
const TTS_OUTPUT_DIR = path.join(UPLOADS_DIR, 'tts_output');
...
const AUDIO_SEPARATOR_CLI_PATH = process.env.MDX_CPU_EXECUTABLE_PATH
  || path.join(MDX_SEPARATOR_DIR, 'mdx-separator.exe');
const AUDIO_SEPARATOR_CUDA_CLI_PATH = process.env.MDX_CUDA_EXECUTABLE_PATH
  || path.join(MDX_SEPARATOR_DIR, 'cuda', 'mdx-separator.exe');
...
function ensureDataDirectories() {
  const dirs = [appDataRoot, DOWNLOADS_DIR, UPLOADS_DIR, COOKIES_DIR, VOICES_DIR, VOICE_PREVIEWS_DIR, TTS_OUTPUT_DIR, MUSIC_DIR, LOGOS_DIR, SUBTITLES_DIR, TMP_UPLOADS_DIR, RENDERS_DIR, PROJECTS_DIR, MODELS_DIR, RENDER_JOBS_DIR, DATA_TOOLS_DIR];
...
const FFPROBE_PATH = getExtPath('tools', 'ffprobe.exe');
...
function getPlatformFromUrl(url) {
  ...
  if (url.includes('rednote.com')) return 'rednote';
...
function getCookieStatus() {
  const platforms = ['bilibili', 'douyin', 'tiktok', 'youtube', 'facebook', 'instagram', 'xiaohongshu', 'rednote', 'weibo', 'youku', 'mgtv', 'iq'];
  const status = {};
...
const CRAWL_VIDEO_PLATFORMS = Object.freeze({
  youtube: 'youtube', tiktok: 'tiktok', facebook: 'facebook', instagram: 'instagram',
  douyin: 'douyin', bili: 'bilibili', xhs: 'xiaohongshu', rednote: 'rednote', honggo: 'honggo'
});
const CRAWL_VIDEO_MODES = Object.freeze({ link: 'detail', 'tu-khoa': 'search', kenh: 'creator', bo: 'chase' });

function listVideoFiles(dir, exts = ['.mp4', '.mov', '.mkv', '.webm']) {
  const root = path.resolve(dir);
  const allowed = new Set(exts.map((ext) => String(ext).toLowerCase()));
  const files = [];
  const pending = [root];
  while (pending.length) {
    ...
    for (const entry of entries) {
      ...
      if (!entry.isFile() || entry.name.startsWith('_temp_') || !allowed.has(path.extname(entry.name).toLowerCase())) continue;
      let stat;
      try { stat = fs.statSync(fullPath); } catch (_) { continue; }
      if (stat.size <= 0) continue;
      const relativePath = path.relative(root, fullPath);
      const parts = relativePath.split(path.sep);
      const crawlPlatform = CRAWL_VIDEO_PLATFORMS[String(parts[0] || '').toLowerCase()];
      const standardCrawlerVideo = Boolean(crawlPlatform && parts[1]?.toLowerCase() === 'videos');
      const directHonggoVideo = crawlPlatform === 'honggo' && parts.length >= 3
        && !['videos', 'jsonl', '.hgstate'].includes(String(parts[1] || '').toLowerCase());
      const isCrawlerVideo = standardCrawlerVideo || directHonggoVideo;
      const parsedStem = path.parse(entry.name).name;
      const isSplitFolder = parts.some((part) => ['clip_nho', 'bam_nho'].includes(part.toLowerCase()));
      const isSplitName = /(?:_clip_|_cảnh)\d+(?:_\d+x\d+|_[a-z0-9]+)?$/i.test(parsedStem);
      files.push({
        ...
        size: stat.size,
        modified: stat.mtime,
        platform: isCrawlerVideo ? crawlPlatform : 'local',
        mode: isCrawlerVideo ? (directHonggoVideo ? 'chase' : (CRAWL_VIDEO_MODES[String(parts[2] || '').toLowerCase()] || 'detail')) : 'local',
        source: directHonggoVideo ? parts.slice(1, -1).join(' / ')
          : (isCrawlerVideo && parts.length > 4 ? parts.slice(3, -1).join(' / ') : ''),
        isSplitClip: isSplitFolder && isSplitName,
        fullPath
      });
    }
  }
  ...
  for (const file of files) {
    if (!file.isSplitClip) continue;
    const clipStem = path.parse(file.name).name.replace(/(?:_clip_|_cảnh)\d+(?:_\d+x\d+|_[a-z0-9]+)?$/i, '');
    completed.add(`${path.dirname(path.dirname(file.fullPath))}\u0000${clipStem}`.toLowerCase());
  }
  const publicFiles = files.map((file) => {
    const sourceKey = `${path.dirname(file.fullPath)}\u0000${path.parse(file.name).name}`.toLowerCase();
    const { fullPath, ...publicFile } = file;
    return { ...publicFile, chopped: !file.isSplitClip && completed.has(sourceKey), _sourceKey: sourceKey };
  });
  const sourcesByKey = new Map(publicFiles.filter((file) => !file.isSplitClip).map((file) => [file._sourceKey, file]));
  const groupedClips = new Set();
  for (const clip of publicFiles.filter((file) => file.isSplitClip)) {
    const clipStem = path.parse(clip.name).name.replace(/(?:_clip_|_cảnh)\d+(?:_\d+x\d+|_[a-z0-9]+)?$/i, '');
    const parentKey = `${path.dirname(path.dirname(path.join(root, clip.relativePath)))}\u0000${clipStem}`.toLowerCase();
    const source = sourcesByKey.get(parentKey);
    if (!source) continue;
    if (!Array.isArray(source.splitClips)) source.splitClips = [];
    source.splitClips.push(clip);
    groupedClips.add(clip.relativePath.toLowerCase());
  }
  for (const source of publicFiles) {
    if (source.splitClips) {
      source.splitClips.sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true }));
      source.splitClipCount = source.splitClips.length;
    }
  }
  return publicFiles
    .filter((file) => !file.isSplitClip || !groupedClips.has(file.relativePath.toLowerCase()))
    .map(({ _sourceKey, ...file }) => ({
      ...file,
      splitClips: file.splitClips?.map(({ _sourceKey: ignored, ...clip }) => clip)
    }))
    .sort((a, b) => new Date(b.modified) - new Date(a.modified));
}
...
function resolveAssetPath(kind, filename) {
  ...
  const safeFilename = kind === 'video' ? String(filename) : path.basename(filename);
  if (!safeFilename || safeFilename === '.' || safeFilename === '..' || path.isAbsolute(safeFilename)) return null;
  const resolved = path.resolve(rootPath, safeFilename);
  if (!resolved.startsWith(`${rootPath}${path.sep}`)) return null;
  if (kind === 'video' && !['.mp4', '.mov', '.mkv', '.webm'].includes(path.extname(resolved).toLowerCase())) return null;
  if (!fs.existsSync(resolved)) return null;
  try {
    if (!fs.statSync(resolved).isFile()) return null;
    const realRoot = fs.realpathSync(rootPath);
    const realResolved = fs.realpathSync(resolved);
    if (!realResolved.startsWith(`${realRoot}${path.sep}`)) return null;
    return realResolved;
  } catch (_) {
    return null;
  }
