...
function killProcessTree(proc) {
  ...
  // Remove listener để Promise không treo chờ 'close' không bao giờ đến
  try { proc.removeAllListeners('close'); proc.removeAllListeners('exit'); } catch (e) {}
}
...
async function runOmnivoiceCLI(args, options = {}, omiDevice = 'cpu') {
  ...
  try {
    ...
    if (!state.isStudioRendering || (err.message && err.message.includes('hủy'))) {
      throw err;
    }
