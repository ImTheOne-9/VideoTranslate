...
function ensureDataDirectories() {
  const dirs = [appDataRoot, DOWNLOADS_DIR, UPLOADS_DIR, COOKIES_DIR, VOICES_DIR, VOICE_PREVIEWS_DIR, MUSIC_DIR, LOGOS_DIR, SUBTITLES_DIR, TMP_UPLOADS_DIR, RENDERS_DIR, PROJECTS_DIR, MODELS_DIR, RENDER_JOBS_DIR, DATA_TOOLS_DIR];
...
function listVideoFiles(dir, exts = ['.mp4', '.mov', '.mkv', '.webm']) {
  ...
  return files.map((file) => {
    ...
    return { ...publicFile, chopped: !file.isSplitClip && completed.has(sourceKey) };
  }).sort((a, b) => new Date(b.modified) - new Date(a.modified));
}
