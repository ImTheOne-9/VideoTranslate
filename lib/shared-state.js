...
const CRAWL_VIDEO_PLATFORMS = Object.freeze({
  ...
  douyin: 'douyin', bili: 'bilibili', xhs: 'xiaohongshu', rednote: 'rednote'
});
...
function listVideoFiles(dir, exts = ['.mp4', '.mov', '.mkv', '.webm']) {
  ...
  while (pending.length) {
    ...
    for (const entry of entries) {
      ...
      const isCrawlerVideo = Boolean(crawlPlatform && parts[1]?.toLowerCase() === 'videos');
      ...
      files.push({
        ...
        mode: isCrawlerVideo ? (CRAWL_VIDEO_MODES[String(parts[2] || '').toLowerCase()] || 'detail') : 'local',
        source: isCrawlerVideo && parts.length > 4 ? parts.slice(3, -1).join(' / ') : '',
