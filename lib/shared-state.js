...
// Thư mục tools cho dependency tải SAU (audio-separator.exe, CUDA DLLs)
...
// [Da bo] whisper_onnx.exe khong con dung sau khi chuyen sang whisper.cpp (ggml)
const WHISPER_CLI_PATH = getExtPath('tools', 'whisper.cpp', 'Release', 'whisper-cli.exe');
const AUDIO_SEPARATOR_CLI_PATH = path.join(DATA_TOOLS_DIR, 'audio-separator.exe');

// Đảm bảo toàn bộ thư mục dữ liệu tồn tại trước khi app dùng (fix lỗi ENOENT khi cài mới)
function ensureDataDirectories() {
  const dirs = [appDataRoot, DOWNLOADS_DIR, UPLOADS_DIR, COOKIES_DIR, VOICES_DIR, MUSIC_DIR, SUBTITLES_DIR, TMP_UPLOADS_DIR, RENDERS_DIR, PROJECTS_DIR, MODELS_DIR, DATA_TOOLS_DIR];
...
// Seed: copy ggml-base + VAD (bundle trong installer) sang MODELS_DIR neu chua co
function seedBundledWhisperModels() {
  try {
    const seedDir = getExtPath('tools', 'whisper.cpp', 'seed-models');
    const seeds = [
      { src: path.join(seedDir, 'ggml-base', 'ggml-base.bin'), dst: path.join(MODELS_DIR, 'whisper', 'ggml-base', 'ggml-base.bin') },
      { src: path.join(seedDir, 'vad', 'ggml-silero-v6.2.0.bin'), dst: path.join(MODELS_DIR, 'whisper', 'vad', 'ggml-silero-v6.2.0.bin') }
    ];
    for (const s of seeds) {
      if (!fs.existsSync(s.src)) continue;
      if (fs.existsSync(s.dst) && fs.statSync(s.dst).size > 0) continue;
      fs.mkdirSync(path.dirname(s.dst), { recursive: true });
      fs.copyFileSync(s.src, s.dst);
      console.log('[Seed] Da copy ' + path.basename(s.dst) + ' sang MODELS_DIR');
    }
  } catch (e) {
    console.warn('[Seed] Bo qua seed model:', e.message);
  }
}
...
module.exports = {
  ...
  TOOLS_DIR,
  DATA_TOOLS_DIR,
  // [Da bo] WHISPER_ONNX_PATH (whisper.cpp/ggml),
  WHISPER_CLI_PATH,
  seedBundledWhisperModels,
