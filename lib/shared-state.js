...
function listVideoFiles(dir, exts = ['.mp4', '.mov', '.mkv', '.webm']) {
  ...
  while (pending.length) {
    ...
    for (const entry of entries) {
      ...
      files.push({
        ...
        isSplitClip: parts.some((part) => part.toLowerCase() === 'clip_nho') && /_clip_\d+(?:_\d+x\d+)?$/i.test(path.parse(entry.name).name),
        fullPath
      });
    }
  }
  ...
  for (const file of files) {
    ...
    const clipStem = path.parse(file.name).name.replace(/_clip_\d+(?:_\d+x\d+)?$/i, '');
  ...
  for (const clip of publicFiles.filter((file) => file.isSplitClip)) {
    const clipStem = path.parse(clip.name).name.replace(/_clip_\d+(?:_\d+x\d+)?$/i, '');
