...
class FacebookPublishQueue {
  ...
  async tick() {
    ...
    const due = this.jobStore.list({ limit: Number.MAX_SAFE_INTEGER }).filter((job) =>
      ['queued', 'retrying', 'processing', 'finalizing', 'facebook_scheduled'].includes(job.status) && new Date(job.nextAttemptAt || job.scheduledAt || 0).getTime() <= Date.now());
  ...
  async runJob(input) {
    ...
    if (job.status === 'facebook_scheduled' && new Date(job.nextAttemptAt || job.scheduledAt).getTime() > Date.now()) return;
    ...
    try {
      ...
      if (!canCheckPublication(job)) {
        ...
        if (job.type === 'feed') {
          ...
          const options = { signal, state: job.upload,
            scheduledPublishTime: job.scheduleMode === 'facebook' ? Math.floor(new Date(job.scheduledAt).getTime() / 1000) : 0,
            onCheckpoint: async (upload) => {
  ...
  deferCheck(job, message = 'Facebook đang xử lý hoặc chưa xác nhận xuất bản') {
    ...
    return this.jobStore.update(job.id, {
      ...
      error: statusChecks >= this.maxStatusChecks
        ? job.scheduleMode === 'facebook' && !job.scheduleConfirmedAt
          ? 'Chưa xác nhận Facebook đã lưu lịch. Hãy kiểm tra trong Meta Business Suite hoặc bấm Kiểm tra lại; không tạo video mới.'
          : 'Chưa xác nhận xuất bản. Bấm kiểm tra lại để kiểm tra video cũ, không upload lại.'
        : message,
  ...
  async checkPublication(client, job, update) {
    ...
    const status = await client.getVideoStatus(job.mediaId || job.upload?.videoId || job.platformWorkId, job.type, { checkSchedule: nativeSchedule });
    ...
    const expectedTime = Math.floor(new Date(job.scheduledAt).getTime() / 1000);
    if (status.published === true) {
      if (nativeSchedule && Date.now() < expectedTime * 1000) update({ warning: 'Facebook báo video đã xuất bản trước giờ hẹn. Hãy kiểm tra bài trên Page.' });
      return status;
    }
    if (nativeSchedule) {
      if (status.scheduledPublishTime && status.scheduledPublishTime !== expectedTime) {
        throw needsReview('Giờ hẹn Facebook trả về khác giờ đã chọn. Hãy kiểm tra lịch trong Meta Business Suite; phần mềm không tự gửi lại.');
      }
      if (status.scheduled === true && status.scheduledPublishTime === expectedTime && Date.now() < expectedTime * 1000) {
        update({ status: 'facebook_scheduled', percent: 100, error: null, statusChecks: 0,
          scheduleConfirmedAt: job.scheduleConfirmedAt || new Date().toISOString(),
          nextAttemptAt: new Date(expectedTime * 1000).toISOString(),
          platformWorkId: status.postId || job.platformWorkId,
          permalink: facebookPermalink(status.permalink || job.permalink) });
        return null;
      }
    }
    this.deferCheck(this.jobStore.get(job.id), nativeSchedule && !job.scheduleConfirmedAt
      ? 'Đang chờ Facebook xác nhận xử lý xong và lưu đúng lịch đăng. Chưa thể tắt phần mềm.'
      : undefined);
    return null;
  }
  ...
  cancel(id) {
    ...
    // Stopping local tracking cannot revoke a schedule already sent to Facebook.
    if (job.scheduleMode === 'facebook' && canCheckPublication(job)) return null;
