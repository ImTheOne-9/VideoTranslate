...
function probeChineseHardsub({
  ...
  if (!status.ready) {
    throw technicalError(
      'Runtime RapidOCR chưa sẵn sàng để dò hardsub',
      status.missing.join(', ')
    );
  }
  const args = [
    ...
    '--probe-only',
    '--probe-frames', String(Math.max(6, Number(frames) || 20)),
    '--probe-min-boxes', String(Math.max(1, Number(minimumBoxes) || 3)),
    '--model', model,
    '--device', ['cpu', 'gpu'].includes(device) ? device : 'auto'
  ];
  const runSpawn = spawnImpl || spawn;
  return new Promise((resolve, reject) => {
    ...
    const onLine = (line) => {
      try {
        const event = JSON.parse(line);
        if (event.stage === 'probe_result') probeResult = event;
        if (event.stage === 'log') onProgress?.({ kind: 'log', ...event });
        if (event.stage === 'error') diagnostic = `${diagnostic}${event.message || ''}\n`.slice(-MAX_DIAGNOSTIC_LENGTH);
      } catch {
    ...
    try {
      ...
      child.stderr?.on('data', (chunk) => {
        diagnostic = `${diagnostic}${String(chunk || '')}`.slice(-MAX_DIAGNOSTIC_LENGTH);
      });
      child.once('error', (error) => settle(() => reject(technicalError('Không thể chạy thăm dò RapidOCR', error.message))));
      child.once('close', (code) => {
        if (code === 0 && probeResult) {
          settle(() => resolve({
            hasHan: probeResult.hasHan === true,
            conclusive: probeResult.conclusive !== false,
            frames: Number(probeResult.frames) || frames,
            minimumBoxes: Number(probeResult.minimumBoxes) || minimumBoxes
          }));
          return;
        }
        if (code === 3) {
          settle(() => resolve({ hasHan: false, conclusive: false, frames, minimumBoxes }));
          return;
        }
        settle(() => reject(technicalError(`Thăm dò RapidOCR thoát mã ${code}`, diagnostic)));
      });
      timer = setTimeout(() => {
        try { shared.killProcessTree(child); } catch {}
        settle(() => reject(technicalError('Thăm dò RapidOCR quá hạn', diagnostic)));
      }, timeoutMs);
    } catch (error) {
      settle(() => reject(technicalError('Không thể chạy thăm dò RapidOCR', error.message)));
    }
  });
}
