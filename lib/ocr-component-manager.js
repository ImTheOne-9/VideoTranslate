...
function inspectInstallation(dataToolsDir, componentDir) {
  try {
...
function createOcrComponentManager(options = {}) {
  ...
  function localInstallation() {
    return inspectInstallation(dataToolsDir, componentDir);
  }
  ...
  async function installAtomically(stagedComponentDir, manifest, backupDir, signal) {
    ...
    try {
      ...
      throwIfAborted(signal);
      const installed = inspectInstallation(dataToolsDir, componentDir);
      throwIfAborted(signal);
