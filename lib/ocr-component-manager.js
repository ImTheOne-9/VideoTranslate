...
function validateManifest(value) {
  ...
  if (!isSafeRelativePath(value.componentRoot) || value.componentRoot !== COMPONENT_ROOT) {
    throw manifestError(`componentRoot must be exactly ${COMPONENT_ROOT}`);
  }
  if (!isSafeRelativePath(value.executable) || value.executable !== EXECUTABLE_NAME) {
    throw manifestError(`executable must be exactly ${EXECUTABLE_NAME}`);
  }
  if (
  ...
  if (
    ...
    !REQUIRED_LANGUAGES.every((language) => value.supportedLanguages.includes(language))
  ) {
    throw manifestError(`supportedLanguages must include ${REQUIRED_LANGUAGES.join(', ')}`);
  }
...
function inspectInstallation(componentDir) {
  try {
    ...
    const manifest = validateManifest(installed);
    if (
...
async function extractZipSafely(archivePath, stagingDir, installedSize, signal) {
  ...
  for (const { entry, destination } of entries) {
    ...
    const contents = await readZipEntry(entry);
    throwIfAborted(signal);
    if (contents.length > installedSize - extractedBytes) {
      throw new Error(`ZIP payload exceeds installedSize of ${installedSize} bytes`);
    }
...
function createOcrComponentManager(options = {}) {
  ...
  const componentDir = path.join(dataToolsDir, COMPONENT_ROOT);
  const executablePath = path.join(componentDir, EXECUTABLE_NAME);
  ...
  function localInstallation() {
    return inspectInstallation(componentDir);
  }
  ...
  async function installAtomically(stagedComponentDir, manifest, backupDir, signal) {
    ...
    try {
      ...
      const installed = inspectInstallation(componentDir);
      ...
      if (oldMoved && await pathExists(backupDir)) {
        await rename(backupDir, componentDir);
      }
      throw error;
    }
