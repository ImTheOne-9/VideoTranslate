...
class DownloadCrawlManager {
  ...
  async _preview(body = {}, observer = {}) {
    ...
    const anonymousDetail = mode === 'detail' && this.anonymousDetailPlatforms.has(platform);
  ...
  async _runCrawlTask(task) {
    task.status = 'downloading';
    task.startedAt = new Date().toISOString();
    const anonymousDetail = task.config?.mode === 'detail'
      && this.anonymousDetailPlatforms.has(task.platform);
    ...
    this._saveTasks();
    try {
      if (!anonymousDetail && this.loginChecker) {
      ...
      // Theo link công khai phải giống Tải đơn lẻ: thử yt-dlp/extractor trước,
      // không ép chạy MediaCrawler (vốn cần profile đăng nhập).
      ...
      const result = await this.preview(task.config || {}, anonymousDetail ? {
      ...
      for (let index = 0; index < items.length; index += 1) {
        ...
        const child = {
          ...
          quality: task.platform === 'bilibili' ? normalizeBilibiliQuality(task.config?.quality) : '',
  ...
  _download(task) {
    return new Promise((resolve) => {
      ...
      const formatSelector = task.platform === 'bilibili'
