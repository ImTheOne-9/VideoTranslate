...
const { selectItems, hasFilters, metric } = require('./crawl-selection');
const { redactLog, monitorProcess } = require('./crawl-process-monitor');
const { recordCrawlerOrigins } = require('./crawler-history-reader');
const { normalizeBilibiliQuality, bilibiliFormatSelector } = require('./mediacrawler-adapter');
const { validateMediaFile, findValidatedOutput } = require('./media-file-validator');

const DEFAULT_PLATFORM_CAPABILITIES = Object.freeze({
  youtube: { search: true, detail: true, creator: true, chase: true, previewModes: ['search', 'detail', 'creator', 'chase'] },
  tiktok: { search: false, detail: true, creator: true, chase: true, login: true, previewModes: ['detail', 'creator'] },
  douyin: { search: true, detail: true, creator: true, chase: true, login: true, previewModes: ['search', 'detail', 'creator'] },
  facebook: { search: false, detail: true, creator: true, chase: false, previewModes: ['detail', 'creator'] },
  bilibili: { search: true, detail: true, creator: true, chase: true, login: true, previewModes: ['search', 'detail', 'creator'] },
  xiaohongshu: { search: true, detail: true, creator: true, chase: false, login: true, previewModes: ['search', 'detail', 'creator'] },
  rednote: { search: true, detail: true, creator: true, chase: false, login: true, previewModes: ['search', 'detail', 'creator'] },
  weibo: { search: true, detail: true, creator: true, chase: false, login: true, previewModes: ['search', 'detail', 'creator'] },
  instagram: { search: false, detail: true, creator: true, chase: false, login: true, previewModes: ['detail', 'creator'] },
  twitter: { search: false, detail: true, creator: true, chase: false, login: true, previewModes: ['detail', 'creator'] },
  reddit: { search: true, detail: true, creator: true, chase: false, previewModes: ['detail', 'creator'] },
  bilitv: { search: false, detail: true, creator: false, chase: false, previewModes: ['detail'] },
  kuaishou: { search: true, detail: true, creator: true, chase: false, login: true, previewModes: ['search', 'detail', 'creator'] },
  honggo: { search: false, detail: true, creator: false, chase: true, previewModes: ['detail', 'chase'] }
});

// Link video công khai của các nền tảng này dùng cùng đường yt-dlp/extractor
// với "Tải đơn lẻ". Chỉ search/creator mới cần MediaCrawler + login gate.
const ANONYMOUS_DETAIL_PLATFORMS = Object.freeze([
  'douyin', 'bilibili', 'xiaohongshu', 'rednote', 'bilitv'
]);

const AUTHENTICATED_DETAIL_PLATFORMS = Object.freeze([
  'douyin', 'bilibili', 'xiaohongshu', 'rednote'
]);

const PLATFORM_OUTPUT_FOLDERS = Object.freeze({
  douyin: 'douyin',
  bilibili: 'bili',
  xiaohongshu: 'xhs',
  rednote: 'rednote',
...
function safeOutputSegment(value, fallback = 'khac') {
...
function resolvePlatformOutputDir(downloadsDir, platform, config = {}) {
  const leaf = PLATFORM_OUTPUT_FOLDERS[String(platform || '').toLowerCase()]
    || safeOutputSegment(platform, 'khac');
  const mediaRoot = path.join(path.resolve(downloadsDir), leaf, 'videos');
  const sourceMode = String(config.sourceMode || config.mode || 'detail').toLowerCase();
  if (sourceMode === 'search') {
    return path.join(mediaRoot, 'tu-khoa', safeOutputSegment(config.sourceInput || config.input));
  }
  if (sourceMode === 'creator') {
    return path.join(mediaRoot, 'kenh', safeOutputSegment(config.sourceName || config.sourceInput, 'khong-ro'));
  }
  if (sourceMode === 'chase') {
    return path.join(mediaRoot, 'bo', safeOutputSegment(config.sourceName || config.sourceInput || config.input));
  }
  return path.join(mediaRoot, 'link');
}
...
function classifyCrawlError(message) {
  ...
  if (/enospc|no space left|disk full|hết dung lượng/.test(text)) return 'disk_full';
  if (/econnreset|enotfound|network|mạng|connection reset/.test(text)) return 'network';
  ...
  if (/tải 0\/\d+ video xhs\/rednote/.test(text)) return 'extractor';
...
class DownloadCrawlManager {
  constructor(options = {}) {
    ...
    this.anonymousPreviewResolvers = options.anonymousPreviewResolvers || {};
    ...
    this.anonymousDetailPlatforms = new Set(
      options.anonymousDetailPlatforms || ANONYMOUS_DETAIL_PLATFORMS
    );
    this.authenticatedDetailPlatforms = new Set(
      options.authenticatedDetailPlatforms || AUTHENTICATED_DETAIL_PLATFORMS
    );
  ...
  _loadHistory() {
    try {
      ...
      return parsed.filter((entry) => {
        if (entry?.status !== 'success' || !entry.outputPath || !path.extname(String(entry.outputPath))) return true;
        return validateMediaFile(entry.outputPath).valid;
      });
    } catch (_) {
      return [];
    }
  ...
  _assertPreviewSupported(platform, mode) {
    this._assertSupported(platform, mode);
    const previewModes = this.platformCapabilities[platform]?.previewModes;
    if (Array.isArray(previewModes) && !previewModes.includes(mode)) {
      const labels = { search: 'theo từ khóa', detail: 'theo link', creator: 'theo kênh', chase: 'theo bộ' };
      throw new Error(`${platform} có thể cào ${labels[mode] || mode}, nhưng chưa hỗ trợ xem trước ổn định ở chế độ này. Hãy dùng Cào hết hoặc Thêm vào hàng đợi.`);
    }
  }
  ...
  async _useAnonymousDetail(platform, mode, forcedValue) {
    if (mode !== 'detail' || !this.anonymousDetailPlatforms.has(platform)) return false;
    if (typeof forcedValue === 'boolean') return forcedValue;
    if (!this.authenticatedDetailPlatforms.has(platform)) return true;
    if (typeof this.crawlResolvers[platform] !== 'function') return true;
    if (!this.loginChecker) return true;
    try {
      return (await this.loginChecker(platform, mode)) !== 'in';
    } catch (_) {
      // Không xác minh chắc chắn được phiên thì giữ đường tải công khai,
      // tránh làm mất chức năng tải đơn lẻ.
      return true;
    }
  }

  _historyHasFile(key) {
    return this.history.some((entry) => entry.key === key && entry.status === 'success'
      && (!entry.outputPath || validateMediaFile(entry.outputPath).valid));
  }

  async preview(body = {}, observer = {}) {
    this.previewActiveCount += 1;
    try {
      return await this._preview(body, observer);
    } finally {
      this.previewActiveCount = Math.max(0, this.previewActiveCount - 1);
    }
  }

  async _preview(body = {}, observer = {}) {
    const request = normalizeCrawlRequest(body);
    const platform = request.platform;
    const mode = request.mode;
    const anonymousDetail = await this._useAnonymousDetail(platform, mode, observer.anonymousDetail);
    if (observer.allowUnsupportedPreview || anonymousDetail) this._assertSupported(platform, mode);
    else this._assertPreviewSupported(platform, mode);
    const count = safeCount(request.count, 20);
    const deepNew = request.deepNew === true && ['search', 'creator'].includes(mode);
    ...
    const resolverKey = `${platform}:${mode}`;
    const resolver = observer.forceGeneric
      ? null
      : anonymousDetail
        ? this.anonymousPreviewResolvers[resolverKey]
        : this.previewResolvers[resolverKey];
    if (resolver) {
      ...
      const emitItem = (item) => {
        ...
        const normalized = {
          ...
          key,
          platform,
          downloaded: Boolean(item.downloaded) || this._historyHasFile(key)
        };
        if (!selectItems([normalized], { ...request, deepNew }).length || streamedKeys.has(key) || streamedKeys.size >= count) return;
        streamedKeys.add(key);
        observer.onItem?.(normalized);
      };
      const resolved = await resolver({
        input: request.input, count: scanCount, platform, mode,
        sort: request.sort, timeDays: request.timeDays, language: request.language, deepNew,
        onItem: emitItem,
        onLog: (line, level = 'info') => this._log(`[Preview ${platform}] ${String(line || '').trim()}`, level)
      });
      ...
      const selected = selectItems(resolvedItems, { ...request, deepNew }).slice(0, count);
      for (const item of selected) emitItem(item);
    ...
    const { sources, flat } = this._sourceForPreview(platform, mode, request.input, scanCount);
    const items = [];
    ...
    for (const source of sources) {
      ...
      for (const line of String(output || '').split(/\r?\n/)) {
        ...
        items.push({
          ...
          creatorUrl: item.channel_url || item.uploader_url || item.channel_follower_count_url || '',
          creatorAvatar: item.channel_thumbnail || item.uploader_avatar || '',
          viewCount: metric(item.view_count),
          likeCount: metric(item.like_count),
          timestamp: Number(item.timestamp || item.release_timestamp) || 0,
          downloaded: this._historyHasFile(key)
        });
    ...
    const sort = String(request.sort || 'relevance');
    ...
    const selected = selectItems(items, { ...request, deepNew }).slice(0, count);
  ...
  enqueue(body = {}) {
    ...
    for (const raw of rawItems.slice(0, 500)) {
      ...
      if (skipDuplicates && this._historyHasFile(key)) {
        skipped += 1;
        continue;
      }
      const task = {
        ...
        quality: platform === 'bilibili' ? normalizeBilibiliQuality(body.quality || raw.quality) : '',
        outputDir,
  ...
  enqueueJob(body = {}) {
    ...
    const count = safeCount(request.count, 100);
    const sourceMode = ['search', 'creator', 'detail', 'chase'].includes(String(request.sourceMode || '').toLowerCase())
      ? String(request.sourceMode).toLowerCase()
      : mode;
    const sourceInput = String(request.sourceInput || (sourceMode === mode ? input : '')).trim();
    const selectedEpisodes = platform === 'honggo' && Array.isArray(body.selectedEpisodes)
      ? body.selectedEpisodes.slice(0, 500).map((item) => ({
        url: String(item?.url || ''),
        seriesId: String(item?.seriesId || ''),
        episodeNumber: Math.max(0, Number.parseInt(item?.episodeNumber, 10) || 0)
      })).filter((item) => /^https:\/\//i.test(item.url) && /^\d{15,}$/.test(item.seriesId) && item.episodeNumber > 0)
      : [];
    const task = {
      id: crypto.randomUUID(),
      kind: 'crawl',
      platform,
      title: String(request.label || `${platform} · ${mode} · ${input.slice(0, 48)}`),
      config: {
        platform, mode, input, count, sourceMode, sourceInput, selectedEpisodes,
        sourceName: String(request.sourceName || '').trim(),
        sort: String(request.sort || 'relevance'),
        timeDays: Number(request.timeDays || 0),
        minLike: Math.max(0, Number(request.minLike) || 0),
        minView: Math.max(0, Number(request.minView) || 0),
        filterDays: Math.max(0, Number(request.filterDays) || 0),
        // Giống ViralCrawl: chọn mode "Theo bộ" là đủ để tải trọn series.
        // Không phụ thuộc vào một cờ UI phụ có thể bị thiếu ở API client cũ.
        wholeSeries: mode === 'chase',
        sourceChannelId: String(request.sourceChannelId || ''),
        quality: ['bilibili', 'bilitv'].includes(platform) ? normalizeBilibiliQuality(request.quality) : '',
        language: String(request.language || ''),
        deepNew: request.deepNew !== false,
        cookies: String(request.cookies || ''),
        cookiesBrowser: String(request.cookiesBrowser || request.cookies_browser || '')
      },
    ...
    this.tasks.push(task);
    try {
      recordCrawlerOrigins(this.downloadsDir, { platform, input, sourceMode, sourceInput, sourceName: task.config.sourceName });
    } catch (error) {
      this._log(`Không lưu được nguồn phân loại lịch sử: ${error.message}`, 'warn', task.id);
    }
    this._saveTasks();
  ...
  snapshot() {
    ...
    return {
      ...
      previewing: this.previewActiveCount > 0,
  ...
  stats(date = '') {
    ...
    const byPlatform = {};
    const weight = (entry) => entry.kind === 'crawl' ? Math.max(0, Number(entry.completedVideos || 0)) : 1;
  ...
  retry(taskId) {
    ...
    if (!task || !(['error', 'cancelled'].includes(task.status) || (task.status === 'success' && task.reason === 'partial'))) return false;
    task.status = 'pending';
    task.reason = null;
    task.percent = 0;
    task.error = null;
  ...
  retryAll(platform = '', reason = '') {
    let count = 0;
    for (const task of this.tasks) {
      if (!(task.status === 'error' || (task.status === 'success' && task.reason === 'partial')) || (platform && task.platform !== platform) || (reason && task.reason !== reason)) continue;
      task.status = 'pending';
      task.reason = null;
      task.percent = 0;
      task.error = null;
  ...
  resumeAfterLogin(platform) {
    let count = 0;
    for (const task of this.tasks) {
      if (task.kind !== 'crawl' || task.platform !== platform || task.status !== 'error' || task.reason !== 'login_expired') continue;
      task.status = 'pending';
      task.reason = null;
      task.error = null;
      task.percent = 0;
      task.step = 'Đã đăng nhập lại · đang chờ tiếp tục';
      task.completedAt = null;
      count += 1;
    }
    if (count) {
      this._saveTasks();
      this._log(`Đã phục hồi ${count} job ${platform} sau khi đăng nhập lại.`, 'success');
      this._processQueue();
    }
    return count;
  }
  ...
  async _runCrawlTask(task) {
    task.status = 'downloading';
    task.startedAt = new Date().toISOString();
    const hybridDetail = task.config?.mode === 'detail'
      && this.authenticatedDetailPlatforms.has(task.platform);
    const anonymousDetail = await this._useAnonymousDetail(task.platform, task.config?.mode);
    task.step = anonymousDetail
      ? 'Đang thử tải link công khai không cần đăng nhập'
      : hybridDetail
        ? 'Đã đăng nhập · dùng MediaCrawler theo link'
        : 'Đang kiểm tra phiên đăng nhập';
    this.activeTaskId = task.id;
    this._saveTasks();
    try {
      if (fs.statfsSync && fs.existsSync(this.downloadsDir)) {
        const space = fs.statfsSync(this.downloadsDir);
        if (space.bavail * space.bsize < 100 * 1024 * 1024) throw Object.assign(new Error('Ổ tải gần hết dung lượng (dưới 100 MB).'), { reason: 'disk_full' });
      }
      if (!hybridDetail && !anonymousDetail && this.loginChecker) {
      ...
      // Theo link dùng cơ chế lai: có phiên thật → MediaCrawler/browser profile;
      // chưa có hoặc không xác minh được → đường tải đơn lẻ công khai.
      const crawlResolver = anonymousDetail ? null : this.crawlResolvers[task.platform];
      if (crawlResolver) {
        let runConfig = task.config;
        if (hasFilters(task.config)) {
          if (task.config.mode === 'chase') throw new Error('Bộ lọc số liệu chưa hỗ trợ cào series. Hãy bỏ bộ lọc để tải bộ.');
          task.step = 'Đang xem trước và áp dụng điều kiện trước khi tải';
          const preview = await this.preview({ ...task.config, count: Math.min(500, task.config.count * 5) });
          if (task.cancelRequested) throw Object.assign(new Error('Đã hủy job.'), { reason: 'cancelled' });
          const chosen = preview.items.slice(0, task.config.count);
          task.found = chosen.length;
          if (!chosen.length) {
            task.status = 'success'; task.percent = 100; task.reason = 'no_matches';
            task.step = 'Không có video đủ dữ liệu và khớp bộ lọc trong phạm vi đã quét';
            return;
          }
          runConfig = { ...task.config, mode: 'detail', input: chosen.map((item) => item.sourceUrl || item.url).join('\n'), count: chosen.length };
        }
        task.step = task.config?.deepNew
          ? 'Đang cào, bỏ qua nội dung đã tải trong phạm vi nền tảng hỗ trợ'
          : 'Đang cào và tải nội dung';
        this._saveTasks();
        const result = await crawlResolver({ ...runConfig, outputDir: this.downloadsDir }, {
          onProcess: (proc) => { this.activeProcess = proc; if (task.cancelRequested) this.shared.killProcessTree(proc); },
          onTimeout: (proc) => this.shared.killProcessTree(proc),
          onLog: (line, level = 'info') => {
            if (task.cancelRequested) return;
            task.step = redactLog(line).trim().slice(0, 240) || task.step;
        ...
        if (task.cancelRequested) throw Object.assign(new Error('Đã hủy job.'), { reason: 'cancelled' });
        if (result?.success === false) throw new Error(result.error || result.message || 'Bộ cào báo thất bại.');
        if (Number(result?.failedVideos) > 0 && !Number(result?.completedVideos)) throw new Error('Tất cả video trong lượt cào đều thất bại.');
        task.status = 'success';
        task.percent = 100;
        ...
        task.failedVideos = Number(result?.failedVideos || 0);
        task.reason = task.failedVideos ? 'partial' : task.completedVideos ? null : result?.skipped ? 'no_new' : 'no_output';
        if (!task.completedVideos) task.step = 'Không tải thêm video mới; kiểm tra phạm vi cào và lịch sử';
        if (task.failedVideos) task.step = `Hoàn tất ${task.completedVideos}, lỗi ${task.failedVideos}`;
      ...
      const result = await this.preview(task.config || {}, {
        anonymousDetail,
        allowUnsupportedPreview: anonymousDetail
      });
      ...
      const downloadOutputDir = anonymousDetail
        ? resolvePlatformOutputDir(this.downloadsDir, task.platform, task.config)
        : this.downloadsDir;
      ...
      for (let index = 0; index < items.length; index += 1) {
        ...
        const child = {
          ...
          quality: ['bilibili', 'bilitv'].includes(task.platform) ? normalizeBilibiliQuality(task.config?.quality) : '',
        ...
        // Bilibili/XHS/RedNote dùng URL gốc + cookie tùy chọn giống tải đơn lẻ.
        // Douyin vẫn cần extractor đổi link trang thành URL CDN trực tiếp.
        const resolver = this.downloadResolvers[child.platform];
        const shouldResolveDownload = resolver && (!anonymousDetail || child.platform === 'douyin');
        if (shouldResolveDownload && !child.resolvedDownload) {
        ...
        if (child.status === 'success') task.completedVideos += 1;
          ...
          task.failedVideos += 1;
          if (child.error) childErrors.push(child.error);
        }
        task.percent = Math.round(((index + 1) / items.length) * 100);
        this.activeTaskId = task.id;
        this._saveTasks();
      }
      if (!task.completedVideos && task.failedVideos) {
        const detail = childErrors.slice(-3).join(' · ');
        throw new Error(`Tất cả video trong job đều tải thất bại${detail ? `: ${detail}` : '.'}`);
      }
      task.status = 'success';
      task.percent = 100;
      task.outputPath = downloadOutputDir;
      ...
      task.error = redactLog(error.message);
      task.reason = error.reason || classifyCrawlError(error.message);
      task.step = task.status === 'cancelled' ? 'Đã hủy' : task.error;
  ...
  _download(task) {
    return new Promise((resolve) => {
      ...
      const formatSelector = ['bilibili', 'bilitv'].includes(task.platform)
        ? bilibiliFormatSelector(task.quality)
      ...
      if (task.platform === 'bilibili') {
        args.push('--http-chunk-size', '8M');
        args.push('--retries', '6');
        args.push('--fragment-retries', '6');
        args.push('--retry-sleep', 'http:exp=5:30');
        args.push('--retry-sleep', 'fragment:exp=2:20');
      }
      ...
      if (['facebook', 'bilibili', 'bilitv'].includes(task.platform)) {
      ...
      let timedOut = false;
      const watchdog = monitorProcess(proc, { diskRoot: task.outputDir, kill: (child) => this.shared.killProcessTree(child),
        onTimeout: (reason) => { timedOut = true; task.reason = reason === 'disk_full' ? reason : 'timeout'; } });
      let lastActivityAt = Date.now();
      const heartbeat = setInterval(() => {
        if (Date.now() - lastActivityAt < 7500 || task.status !== 'downloading') return;
        task.step = task.percent > 0
          ? `Đang tải ${task.percent}% · kết nối vẫn hoạt động`
          : 'Đang lấy luồng hoặc chờ CDN phản hồi';
        this._log(`[Heartbeat] ${task.title}: ${task.step}`, 'info', task.id);
        this._saveTasks();
      ...
      const onLine = (chunk) => {
        ...
        for (const line of String(chunk || '').split(/\r?\n/)) {
          ...
          if (diagnostic && !/^\[?download\]?/i.test(diagnostic)) {
            diagnosticLines.push(diagnostic.slice(0, 300));
      ...
      proc.once('close', (code, signal) => {
        ...
        watchdog.stop();
        if (timedOut) {
          task.status = 'error'; task.error = task.reason === 'disk_full' ? 'Ổ tải gần hết dung lượng' : 'Tải quá thời gian hoặc không còn hoạt động'; task.step = task.error;
        } else if (task.status === 'error') {
          // giữ lỗi đã ghi từ event error
        } else if (code === 0) {
          const verified = findValidatedOutput(task.outputDir, task.filenameBase, Date.parse(task.startedAt || '') || 0);
          if (!verified.valid) {
            task.status = 'error';
            task.step = 'File tải về không hợp lệ';
            task.error = `yt-dlp kết thúc nhưng không tạo được video hợp lệ (${verified.reason})`;
            this._log(`Tải không hợp lệ, không ghi lịch sử: ${task.title} · ${verified.reason}`, 'error', task.id);
          } else {
          task.status = 'success';
          task.percent = 100;
          task.step = 'Đã tải xong';
          task.outputPath = verified.path;
          this.history.push({
          ...
          const diagnostic = diagnosticLines.slice(-3).join(' | ');
          task.error = task.status === 'error'
            ? `yt-dlp dừng với mã ${code}${diagnostic ? `: ${diagnostic}` : ''}`
            : null;
...
module.exports = {
  ...
  ANONYMOUS_DETAIL_PLATFORMS,
  AUTHENTICATED_DETAIL_PLATFORMS,
  PLATFORM_OUTPUT_FOLDERS,
  splitInputs,
  safeCount,
  safeOutputSegment,
