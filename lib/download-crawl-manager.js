...
const DEFAULT_PLATFORM_CAPABILITIES = Object.freeze({
  ...
  tiktok: { search: true, detail: true, creator: true, chase: false, login: true, previewModes: ['search', 'detail', 'creator'] },
  douyin: { search: true, detail: true, creator: true, chase: false, login: true, previewModes: ['search', 'detail', 'creator'] },
    ...
    if (resolver) {
      ...
      const emitItem = (item) => {
        ...
        const normalized = {
          ...
          key,
          platform,
          downloaded: Boolean(item.downloaded) || this.history.some((entry) => entry.key === key && entry.status === 'success')
        };
        if ((deepNew && normalized.downloaded) || streamedKeys.has(key) || streamedKeys.size >= count) return;
  ...
  enqueueJob(body = {}) {
    ...
    const task = {
      ...
      config: {
        ...
        quality: platform === 'bilibili' ? normalizeBilibiliQuality(request.quality) : '',
