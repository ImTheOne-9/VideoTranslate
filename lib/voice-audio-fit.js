...
async function applyVoiceSpeedToFile(options = {}) {
  const inputPath = options.inputPath;
  const speed = Math.max(0.5, Math.min(2, Number(options.speed) || 1));
  if (!inputPath || !fs.existsSync(inputPath) || Math.abs(speed - 1) < 0.001) return false;
  const outputPath = `${inputPath}.voice-speed.tmp.wav`;
  try {
    fs.rmSync(outputPath, { force: true });
    await options.runExecFile(options.ffmpegPath, [
      '-y', '-i', inputPath,
      '-filter:a', buildAtempoFilters(speed).join(','),
      '-c:a', 'pcm_s16le', outputPath
    ]);
    if (!isNarrationAudioUsable(outputPath, { minimumDurationMs: 60 })) {
      throw new Error('Audio sau khi áp tốc độ giọng không hợp lệ');
    }
