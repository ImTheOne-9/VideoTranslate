...
async function detectSpeechRegions(audioPath, options = {}) {
  ...
  for (let offset = 0; offset < samples.length; offset += FRAME_SAMPLES) {
    ...
    if (probability >= config.threshold) {
      ...
      if (silenceFrames >= minSilenceFrames) {
        ...
        if (speechFrames >= minSpeechFrames) rawRegions.push({ startSample: candidateStart, endSample });
        candidateStart = null;
        speechFrames = 0;
        silenceFrames = 0;
      }
  ...
  if (candidateStart != null && speechFrames >= minSpeechFrames) {
