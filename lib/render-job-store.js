...
class RenderJobStore {
  ...
  toManifest(task) {
    ...
    return {
      ...
      sourceSubtitleReview: task.sourceSubtitleReview || null,
      sourceSubtitleReviewApproved: task.sourceSubtitleReviewApproved === true,
  ...
  fromManifest(manifest) {
    return {
      ...
      subtitleTimelineReport: manifest.subtitleTimelineReport || null,
      subtitleSource: manifest.subtitleSource || null,
      voiceExecution: manifest.voiceExecution || null,
      backgroundSeparation: manifest.backgroundSeparation || null,
      segmentReview: manifest.segmentReview || null,
      sourceSubtitleReview: manifest.sourceSubtitleReview || null,
      sourceSubtitleReviewApproved: manifest.sourceSubtitleReviewApproved === true,
  ...
  loadUnfinishedTasks() {
    ...
    for (const name of this.fs.readdirSync(this.rootDir)) {
      try {
        ...
        const preservedWaitingAction = task.status === 'waiting_input'
          && ['ocr_fallback', 'segment_review', 'source_subtitle_review'].includes(task.actionRequired);
        if (!preservedWaitingAction) {
