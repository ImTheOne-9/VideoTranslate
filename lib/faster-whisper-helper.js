  ...
  const args = options.check
    ...
    : options.detectLanguageOnly
      ? [
          ...
          '--sample-seconds', String(Math.max(5, Math.min(60, Number(options.sampleSeconds) || 25)))
        ]
    : [
        runtime.workerPath,
        '--audio', path.resolve(inputPath),
...
async function detectSpokenLanguage(options = {}) {
  const resultPath = options.resultPath || path.join(options.workDir, 'spoken-language.json');
  ...
  await (options.runWorker || runWorker)({
    ...
    model: options.model || 'tiny',
    sampleSeconds: Number(options.sampleSeconds) || 25
  });
  const result = JSON.parse(fs.readFileSync(resultPath, 'utf8'));
  const language = String(result.language || '').trim().toLowerCase();
  if (!language) {
    throw new FasterWhisperError('Faster-Whisper Tiny không trả ngôn ngữ', {
      code: 'FASTER_WHISPER_LANGUAGE_EMPTY'
    });
  }
  return {
    language,
    probability: Number.isFinite(Number(result.languageProbability))
      ? Number(result.languageProbability)
      : 0,
    device: result.device || null,
    model: result.model || options.model || 'tiny',
    sampleSeconds: Number(result.sampleSeconds) || Number(options.sampleSeconds) || 25
  };
}
