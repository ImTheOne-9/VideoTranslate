...
const { EDGE_LANGUAGE_VOICES } = require('../voice-language-catalog');

// Các ngôn ngữ đích đang được Studio cung cấp và OmniVoice hỗ trợ. Ba mã
// không được runtime upstream hỗ trợ (ne/su/ar) được loại từ đầu để caller
// chuyển thẳng sang Edge đúng ngôn ngữ thay vì nạp model rồi mới thất bại.
const SUPPORTED_LANGUAGES = Object.freeze(
  Object.keys(EDGE_LANGUAGE_VOICES).filter(language => !['ne', 'su', 'ar'].includes(language))
);
...
class CurrentOmniVoiceEngine extends VoiceEngine {
  ...
  buildArguments(options = {}) {
    ...
    const requestedLanguage = String(options.language || 'vi').toLowerCase().split(/[-_]/)[0];
    if (!SUPPORTED_LANGUAGES.includes(requestedLanguage)) {
      throw new VoiceEngineError(`OmniVoice chưa hỗ trợ ngôn ngữ '${requestedLanguage}'`, {
        code: 'VOICE_ENGINE_UNSUPPORTED_LANGUAGE',
        engineId: this.id,
        details: { requestedLanguage, supportedLanguages: SUPPORTED_LANGUAGES }
      });
    }
    ...
    const args = [
      ...
      '--language', language,
      '--device', device,
      '--num-step', String(options.steps || 8),
      '--seed', resolveOmnivoiceSeed(options.seed),
      '--position-temperature', String(options.positionTemperature || 1.0)
    ];
