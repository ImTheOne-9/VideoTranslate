...
function normalizeAudioMasteringConfig(body = {}) {
  ...
  return {
    ...
    crossfadeMs: enabled
      ? clamp(body.audioCrossfadeMs, 0, 100, DEFAULT_AUDIO_MASTERING.crossfadeMs)
      : 0,
...
function buildAudioMixGraph(options) {
  ...
  const limiter = `alimiter=limit=${limiterLimit}`
    + ':attack=5:release=50:level=disabled';
  let masterFilters = limiter;
  if (config.enabled) {
    masterFilters = `loudnorm=I=${config.mixLufs}:LRA=${config.loudnessRange}`
      + `:TP=${config.truePeakDb},${limiter}`;
  }
  filters.push(
