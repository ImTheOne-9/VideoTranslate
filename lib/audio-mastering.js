...
function buildAudioMixGraph(options) {
  ...
  filters.push(`[${originalInputLabel}]volume=${Math.max(0, Number(originalVolume) || 0)}[original_pre]`);
  ...
  musicInputs.forEach((input, index) => {
    ...
    filters.push(
      `[${input.inputLabel || `${input.index}:a`}]volume=${Math.max(0, Number(input.volume) || 0)}[${label}]`
    );
