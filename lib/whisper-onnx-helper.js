...
function normalizeWhisperChunks(chunks, durationSeconds) {
...
function runWorker(options) {
  return new Promise((resolve, reject) => {
    const { onStage, onRegionResult, timeoutMs, owner, ...optionData } = options;
    ...
    const timeout = effectiveTimeoutMs > 0 ? setTimeout(() => {
      ...
      finish(() => reject(new Error(`Whisper ONNX timeout sau ${Math.round(effectiveTimeoutMs / 60000)} phút`)));
...
async function transcribeToSrt(options) {
  ...
  const cues = normalizeWhisperChunks(result?.chunks, options.durationSeconds);
