...
(async () => {
  ...
  let samples;
  if (!Array.isArray(workerData.speechRegions)) {
  ...
  if (Array.isArray(workerData.speechRegions)) {
    const chunks = [];
    const texts = [];
    for (let index = 0; index < workerData.speechRegions.length; index += 1) {
      const region = workerData.speechRegions[index];
      parentPort.postMessage({
        type: 'stage',
        stage: 'transcribing_region',
        current: index + 1,
        total: workerData.speechRegions.length
      });
      const regionResult = await transcriber(region.samples, transcribeOptions);
      if (regionResult && regionResult.text) texts.push(String(regionResult.text).trim());
      for (const chunk of Array.isArray(regionResult && regionResult.chunks) ? regionResult.chunks : []) {
        const start = chunk.timestamp && chunk.timestamp[0];
        const end = chunk.timestamp && chunk.timestamp[1];
        chunks.push({
          ...chunk,
          timestamp: [
    ...
    result = { text: texts.filter(Boolean).join(' '), chunks, vad: workerData.vadMetadata };
  } else {
    result = await transcriber(samples, transcribeOptions);
    if (workerData.vadMetadata) result.vad = workerData.vadMetadata;
  }
