...
const FASTER_WHISPER_CONFIG = Object.freeze({
  id: 'faster-whisper',
  repo: 'mobiuslabsgmbh/faster-whisper-large-v3-turbo',
  revision: '0a363e9161cbc7ed1431c9597a8ceaf0c4f78fcf',
  model: 'large-v3-turbo',
  folder: 'faster-whisper',
  localFolder: 'large-v3-turbo',
  files: [
    { name: 'config.json', size: 2263 },
    { name: 'model.bin', size: 1617884929 },
    { name: 'preprocessor_config.json', size: 340 },
    { name: 'tokenizer.json', size: 2710337 },
    { name: 'vocabulary.json', size: 1068114 }
  ]
});

function validateFasterWhisperDirectory(modelDir) {
  const missingFiles = [];
  const invalidFiles = [];
  ...
  const ready = missingFiles.length === 0 && invalidFiles.length === 0;
  return { ready, exists: ready, missingFiles, invalidFiles, modelDir };
}

function getFasterWhisperReadiness(modelsDir) {
  const modelRoot = path.join(modelsDir, 'whisper', FASTER_WHISPER_CONFIG.folder);
  const candidates = [path.join(modelRoot, FASTER_WHISPER_CONFIG.localFolder)];
  const cacheSnapshots = path.join(
    modelRoot,
    `models--${FASTER_WHISPER_CONFIG.repo.replace('/', '--')}`,
    'snapshots'
  );
  if (fs.existsSync(cacheSnapshots)) {
    for (const entry of fs.readdirSync(cacheSnapshots, { withFileTypes: true })) {
      if (entry.isDirectory()) candidates.push(path.join(cacheSnapshots, entry.name));
    }
  }

  let best = validateFasterWhisperDirectory(candidates[0]);
  for (const candidate of candidates) {
    const result = validateFasterWhisperDirectory(candidate);
    if (result.ready) {
      return {
        ...result,
        state: 'ready',
        config: FASTER_WHISPER_CONFIG,
        modelRoot
      };
    }
    if (result.invalidFiles.length > best.invalidFiles.length) best = result;
  }
  return {
    ...best,
    state: best.invalidFiles.length > 0 ? 'corrupt' : 'missing',
    config: FASTER_WHISPER_CONFIG,
    modelRoot
  };
}

async function ensureFasterWhisperModelExist(modelsDir, progressCallback) {
  const readiness = getFasterWhisperReadiness(modelsDir);
  if (readiness.ready) {
    const totalBytes = FASTER_WHISPER_CONFIG.files.reduce((sum, file) => sum + file.size, 0);
    progressCallback?.({
      file: FASTER_WHISPER_CONFIG.model,
      percent: 100,
      downloadedBytes: totalBytes,
      totalBytes
    });
    return readiness.modelDir;
  }

  const targetDir = path.join(
    modelsDir,
    'whisper',
    FASTER_WHISPER_CONFIG.folder,
    FASTER_WHISPER_CONFIG.localFolder
  );
  fs.mkdirSync(targetDir, { recursive: true });
  const totalBytes = FASTER_WHISPER_CONFIG.files.reduce((sum, file) => sum + file.size, 0);
  let overallDownloadedBytes = 0;

  for (const fileInfo of FASTER_WHISPER_CONFIG.files) {
    const destPath = path.join(targetDir, fileInfo.name);
    if (fs.existsSync(destPath) && fs.statSync(destPath).size === fileInfo.size) {
      overallDownloadedBytes += fileInfo.size;
      continue;
    }
    ...
    let fileDownloadedBytes = 0;
    await downloadFile(
      `https://huggingface.co/${FASTER_WHISPER_CONFIG.repo}/resolve/${FASTER_WHISPER_CONFIG.revision}/${encodeURIComponent(fileInfo.name)}`,
      partialPath,
      (downloaded) => {
        overallDownloadedBytes += downloaded - fileDownloadedBytes;
        fileDownloadedBytes = downloaded;
    ...
    if (actualSize !== fileInfo.size) {
      ...
      throw new Error(`Model Faster-Whisper tải chưa đủ: ${fileInfo.name} (${actualSize}/${fileInfo.size} byte)`);
    }
    fs.renameSync(partialPath, destPath);
  }

  const finalReadiness = validateFasterWhisperDirectory(targetDir);
  if (!finalReadiness.ready) {
    throw new Error(`Model Faster-Whisper chưa hợp lệ: ${[
      ...finalReadiness.missingFiles,
      ...finalReadiness.invalidFiles.map((file) => file.name)
    ].join(', ')}`);
  }
  progressCallback?.({
    file: FASTER_WHISPER_CONFIG.model,
...
module.exports = {
  ensureModelsExist,
  ensureFasterWhisperModelExist,
  getFasterWhisperReadiness,
  validateFasterWhisperDirectory,
