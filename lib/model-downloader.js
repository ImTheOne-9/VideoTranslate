...
const WHISPER_MODELS_CONFIG = {
  base:       { repo: 'ggerganov/whisper.cpp', files: [{ name: 'ggml-base.bin',      size: 142  * 1024 * 1024 }] },
  tiny:       { repo: 'ggerganov/whisper.cpp', files: [{ name: 'ggml-tiny.bin',      size: 75   * 1024 * 1024 }] },
  small:      { repo: 'ggerganov/whisper.cpp', files: [{ name: 'ggml-small.bin',      size: 466  * 1024 * 1024 }] },
  medium:     { repo: 'ggerganov/whisper.cpp', files: [{ name: 'ggml-medium.bin',     size: 1530 * 1024 * 1024 }] },
  'large-v3': { repo: 'ggerganov/whisper.cpp', files: [{ name: 'ggml-large-v3.bin',   size: 3000 * 1024 * 1024 }] }
};

async function ensureWhisperModelExist(modelsDir, modelName, progressCallback) {
  const config = WHISPER_MODELS_CONFIG[modelName];
  if (!config) throw new Error(`Model Whisper không hợp lệ: ${modelName}`);

  const whisperDir = path.join(modelsDir, 'whisper', 'ggml-' + modelName);
  if (!fs.existsSync(whisperDir)) {
    fs.mkdirSync(whisperDir, { recursive: true });
  }

  const totalFiles = config.files.length;
  const totalBytes = config.files.reduce((sum, f) => sum + f.size, 0);
  let overallDownloadedBytes = 0;
  let lastPercent = -1;

  for (let i = 0; i < totalFiles; i++) {
    const fileInfo = config.files[i];
    const destPath = path.join(whisperDir, fileInfo.name);
    const fileUrl = `https://huggingface.co/${config.repo}/resolve/main/${fileInfo.name}`;
...
module.exports = { ensureModelsExist, ensureWhisperModelExist, ensureNllbModelExist, WHISPER_MODELS_CONFIG };
