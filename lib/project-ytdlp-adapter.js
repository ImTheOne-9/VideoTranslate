...
function mapPreviewItem(item, platform) {
  ...
  const seriesId = String(item.series_id || (platform === 'honggo' ? id.split('/')[0] : ''));
  const episodeNumber = Number(item.episode ?? item.episode_number) || null;
  return {
    id,
    ...
    episodeNumber,
    episodeCount: Number(item.episode_count) || null,
    platform,
