...
async function resolveBilibiliShortUrls(value, fetchImpl = global.fetch) {
  if (!/b23\.tv\//i.test(String(value || '')) || typeof fetchImpl !== 'function') return String(value || '');
  const parts = splitCrawlerInputs(value);
  const resolved = [];
  for (const part of parts) {
    ...
    try {
      ...
      const match = String(response.url || '').match(/bilibili\.com\/video\/(BV[\w]+|av\d+)/i);
      resolved.push(match ? `https://www.bilibili.com/video/${match[1]}` : part);
    } catch (_) {
      resolved.push(part);
    }
  }
...
class MediaCrawlerAdapter {
  ...
  async preview({ platform, mode, input, count, onLog }) {
    ...
    const result = await this._run(path.join(this.appRoot, 'tim_anh.py'), args, { onLog: previewLog });
  ...
  async crawl(config, hooks = {}) {
    ...
    if (platform === 'bilibili' && mode !== 'search') input = await resolveBilibiliShortUrls(input);
    ...
    if (platform === 'rednote' || platform === 'xiaohongshu') {
      env.MC_XHS_INTL = platform === 'rednote' || process.env.MC_XHS_NOIDIA !== '1' ? '1' : '0';
  ...
  async _crawlXhsBrowser(config, hooks = {}) {
    ...
    const intl = platform === 'rednote' || process.env.MC_XHS_NOIDIA !== '1' ? '1' : '0';
    ...
    let links;
    let outputDir = path.join(mediaRoot, 'kenh');
