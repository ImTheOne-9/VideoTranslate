...
function bilibiliFormatSelector(value) {
  ...
  return `bestvideo${limit}[ext=mp4]+bestaudio[ext=m4a]/bestvideo${limit}+bestaudio/best${limit}`;
}
...
class MediaCrawlerAdapter {
  ...
  async _crawlBilibili(request, hooks = {}) {
    ...
    hooks.onLog?.(`🎞 Bilibili: MediaCrawler đã lấy metadata; yt-dlp sẽ tải tối đa ${qualityLabel} và ghép hình + tiếng.`, 'info');
    ...
    try {
      ...
      for (let index = 0; index < records.length; index += 1) {
        ...
        const before = countMediaFiles(outputDir);
        ...
        try {
          ...
          const created = Math.max(0, countMediaFiles(outputDir) - before);
          ...
          failedVideos += 1;
          hooks.onLog?.(`ERROR Bilibili ${record.id || record.url}: ${error.message}`, 'error');
        }
      }
