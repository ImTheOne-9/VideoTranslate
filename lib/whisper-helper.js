...
// Whisper.cpp CLI
const WHISPER_CLI = getExtPath('tools', 'whisper.cpp', 'Release', 'whisper-cli.exe');
...
function getModelPath(model) {
  ...
  // Fallback 1: ggml-base (model mac dinh duoc bundle trong installer)
  const basePath = path.join(MODELS_DIR, 'whisper', 'ggml-base', 'ggml-base.bin');
  if (fs.existsSync(basePath)) {
    console.warn('[Whisper] Model ' + model + ' chua tai, dung ggml-base thay the');
    return basePath;
  }
  // Fallback 2: bat ky model ggml nao da co san (vd dev chi co ggml-small)
  try {
    const wDir = path.join(MODELS_DIR, 'whisper');
    if (fs.existsSync(wDir)) {
      const subs = fs.readdirSync(wDir).filter(function (n) { return n.indexOf('ggml-') === 0; });
      for (const s of subs) {
        const f = path.join(wDir, s, s + '.bin');
        if (fs.existsSync(f) && fs.statSync(f).size > 0) {
          console.warn('[Whisper] Model ' + model + ' chua tai, dung ' + s + ' thay the');
          return f;
        }
      }
    }
