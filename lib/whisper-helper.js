...
async function extractAudioAndTranscribe(videoPath, tempDir, ffmpegPath, model = 'medium', videoDurationMs, language, onnxVariant = 'medium-q8') {
  ...
  const checkpointKey = createCheckpointSignature({
    ...
    audioFilter: 'highpass=150,lowpass=8000,anlmdn=2'
  });
  const audioMeta = readJsonFile(audioMetaPath);
  const canReuseAudio = audioMeta?.checkpointKey === checkpointKey
