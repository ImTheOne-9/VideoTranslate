...
class FacebookApiService {
  ...
  static async exchangeUserToken(appId, appSecret, shortLivedToken, options = {}) {
    const cleanAppId = String(appId || '').trim();
    const cleanSecret = String(appSecret || '').trim();
    const cleanToken = String(shortLivedToken || '').trim();
    if (!/^\d{5,32}$/.test(cleanAppId)) throw new FacebookApiError('App ID không hợp lệ.', { retryable: false });
    if (!cleanSecret || cleanSecret.length > 512) throw new FacebookApiError('App Secret không hợp lệ.', { retryable: false });
    if (!cleanToken || cleanToken.length > 8192) throw new FacebookApiError('User Access Token không hợp lệ.', { retryable: false });

    const service = new FacebookApiService('', '', options);
    ...
    const response = await service.request({
      method: 'post',
      url: `${service.baseUrl}/oauth/access_token`,
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      data: body.toString()
    }, 'exchangeUserToken');
    const accessToken = String(response.data?.access_token || '').trim();
    if (!accessToken) throw new FacebookApiError('Meta không trả User Access Token dài hạn.', { retryable: false });
