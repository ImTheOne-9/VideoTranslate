...
function normalizeVideoStatus(data = {}, type = 'post') {
  ...
  const published = !failed && processed && (type === 'post'
    ...
    : publicationComplete);
  return { id: data.id, postId: data.post_id || null, status, published, failed,
...
class FacebookApiService {
  ...
  async uploadMedia(type, videoPath, description, title, progress, options) {
    ...
    if (!state.videoId) {
      await checkpoint({ phase: 'starting', type, size, mtimeMs: stat.mtimeMs, apiVersion: this.apiVersion });
      let start;
      try {
    ...
    try {
      finish = await this.request({ method: 'post', url, signal, params: {
        ...
        ...(type === 'post' ? { upload_session_id: state.sessionId, published: true, ...(title ? { title } : {}), ...(description ? { description } : {}) }
          : { video_id: state.videoId, ...(type === 'reel' ? { video_state: 'PUBLISHED', ...(description ? { description } : {}) } : {}) })
      } }, `${endpoint}.finish`);
    } catch (error) {
  ...
  async getVideoStatus(videoId, type = 'post', options = {}) {
    const response = await this.request({
      ...
      params: { fields: type === 'post' ? 'id,status,published,permalink_url,post_id' : 'id,status', access_token: this.accessToken }
    }, 'getVideoStatus');
  ...
  async listComments(postId, limit = 50) {
    const response = await this.request({
      ...
      params: { fields: 'id,message,from,created_time,like_count,comment_count,permalink_url', limit, access_token: this.accessToken }
    }, 'listComments');
    return response.data;
  }
  ...
  async likeObject(objectId, liked = true) {
    await this.request({ method: liked ? 'post' : 'delete', url: `${this.baseUrl}/${objectId}/likes`, params: { access_token: this.accessToken } }, liked ? 'likeObject' : 'unlikeObject');
    return true;
  }
