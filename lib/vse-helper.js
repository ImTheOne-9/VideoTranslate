...
async function runVse({
  ...
  return new Promise((resolve, reject) => {
    ...
    const appendDiagnostic = (text) => {
      diagnosticText = `${diagnosticText}${String(text)}`.slice(-MAX_DIAGNOSTIC_LENGTH);
    };
    ...
    const onError = (error) => {
      ...
      settle(() => reject(createTechnicalError('Unable to start VSE OCR', diagnosticText)));
    };
    ...
    const onClose = (code) => {
      ...
      if (code !== 0) {
        settle(() => reject(createTechnicalError(`VSE OCR exited with code ${code}`, diagnosticText)));
        return;
      }
      if (!fs.existsSync(outputPath)) {
        settle(() => reject(createTechnicalError('VSE OCR completed without the requested output file', diagnosticText)));
        return;
      }
    ...
    try {
      ...
      timer = setTimeout(() => {
        ...
        settle(() => reject(createTechnicalError('VSE OCR timed out', diagnosticText)));
      }, timeoutMs);
    } catch (error) {
      appendDiagnostic(error?.message ?? error);
      settle(() => reject(createTechnicalError('Unable to start VSE OCR', diagnosticText)));
