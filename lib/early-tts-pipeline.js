...
const { voiceSpeedToPiperLengthScale } = require('./voice-speed-policy');
...
class EarlyTtsPipeline {
  constructor(options = {}) {
    ...
    this.voiceSpeed = Number(options.voiceSpeed) || 1;
    this.postProcessAudio = options.postProcessAudio || null;
  ...
  enqueue(items = []) {
    ...
    for (let index = 0; index < items.length; index += 1) {
      ...
      const promise = this.engine.synthesize({
        ...
        language: this.language,
        device: this.device,
        lengthScale: voiceSpeedToPiperLengthScale(this.voiceSpeed)
      }).then(() => {
        if (!fs.existsSync(outputPath) || fs.statSync(outputPath).size <= 44) {
          throw new Error('WAV đọc sớm rỗng');
        }
        return this.postProcessAudio?.(outputPath, this.engine.id);
      }).then(() => {
    ...
    if (batchEntries.length > 0) {
      const promise = this.engine.synthesizeBatch({
        ...
        for (let index = 0; index < results.length; index += 1) {
          ...
          await this.postProcessAudio?.(entry.outputPath, this.engine.id);
