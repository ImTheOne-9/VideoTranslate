...
function normalizeDouyin(platform, mode, input) {
  ...
  if (search && !(['detail', 'chase'].includes(mode) && /[?&](?:vid|modal_id)=\d+/i.test(nextInput))) {
  ...
  if (['detail', 'chase'].includes(nextMode) && /douyin\.com/i.test(nextInput)) {
    ...
    nextInput = (extractHttpUrls(nextInput).length ? extractHttpUrls(nextInput) : [nextInput])
      .map((url) => url.split('?')[0]).join('\n');
  }
  return { mode: nextMode, input: nextInput };
}
...
function normalizeCrawlRequest(body = {}) {
  let platform = String(body.platform || 'youtube').toLowerCase();
  ...
  if (platform === 'bilibili' && mode === 'detail') {
    const urls = extractHttpUrls(input);
    if (urls.length && urls.every((url) => /^https?:\/\/(?:[\w-]+\.)?(?:bilibili\.tv|bstation\.tv)\//i.test(url))) platform = 'bilitv';
  }
