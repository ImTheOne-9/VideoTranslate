...
function normalizeDouyin(platform, mode, input) {
  ...
  if (nextMode === 'detail' && /douyin\.com/i.test(nextInput) && !/\/video\//i.test(nextInput)) {
    const video = nextInput.match(/[?&](?:vid|modal_id)=(\d+)/i) || nextInput.match(/\/note\/(\d+)/i);
    if (video) nextInput = `https://www.douyin.com/video/${video[1]}`;
