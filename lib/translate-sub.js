...
const { translateViaGeminiWeb } = require('./gemini-web-adapter');
...
function createTranslationCheckpoint(outputPath, context) {
  ...
  const signature = createCheckpointSignature({
    // Version 3 adds one whole-SRT analysis shared by every translation batch.
    version: 3,
...
async function translateSubtitles(inputPath, outputPath, geminiApiKey, maxLines = 0, maxCharsPerLine = 22, isCancelled = null) {
  ...
  // 3.5. Dịch bằng Gemini Web Playwright Automation (Node.js Thuần)
  if (aiProvider === 'gemini-web') {
    console.log(`Đang gọi Gemini Web (Playwright) để dịch phụ đề...`);
    try {
      await translateViaGeminiWeb(inputPath, outputPath, {
        targetLang,
        show: false,
        onProgress: (msg) => {
      ...
      const translatedSrtContent = fs.readFileSync(outputPath, 'utf-8');
      const translatedSrtArray = parser.fromSrt(translatedSrtContent);
      const gReport = attachTranslationStats(translatedSrtArray, {
        total: srtArray.length,
        translated: translatedSrtArray.length,
        failed: 0,
        fallbackUsed: 0,
        unchangedFromSource: 0,
        sourceLanguageRemaining: 0,
      ...
      return { outputPath, report: gReport };
    } catch (err) {
      console.error(`⚠ Dịch Gemini Web thất bại (${err.message}). Tự động fallback sang NLLB local...`);
      const quality = await translateWithNllb(inputPath, outputPath, srcLang, maxLines, maxCharsPerLine, isCancelled, nllbTargetLang);
      const gReport = attachTranslationStats(quality, {
        total: srtArray.length,
        translated: srtArray.length,
        failed: 0,
