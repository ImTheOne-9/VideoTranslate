...
async function translateSubtitles(inputPath, outputPath, geminiApiKey, maxLines = 0, maxCharsPerLine = 22, isCancelled = null) {
  ...
  if (aiProvider === 'nllb') {
    await translateWithNllb(inputPath, outputPath, srcLang, maxLines, maxCharsPerLine, isCancelled, nllbTargetLang);
    return;
  }
  ...
  if ((aiProvider === 'gemini' && geminiApiKeyVal && geminiApiKeyVal.trim() !== '') ||
    ...
    for (let i = 0; i < srtArray.length; i += batchSize) {
      ...
      batch.forEach(function (item) {
        let t = preProcessText(item.text);
      ...
      const translateFn = function (m) {
        if (isGemini) return translateWithGemini(m, apiKey, geminiModelVal, targetLangName, prevContext);
        if (isOpenRouter) return translateWithOpenRouter(m, apiKey, openRouterModelVal, targetLangName, prevContext);
        return translateWithNineRouter(m, ninerouterApiKeyVal, ninerouterModelVal, ninerouterBaseUrlVal, targetLangName, prevContext);
      };
      ...
      batch.forEach(function (item) {
        if (translatedMap && translatedMap[item.id]) {
          item.text = postProcessText(translatedMap[item.id]);
        } else {
          console.warn('Thiếu câu dịch cho ID: ' + item.id);
        }
      });

      const translatedItems = batch.filter(function (it) { return translatedMap[it.id]; }).map(function (it) { return { id: it.id, original: batchMap[it.id], translated: translatedMap[it.id] }; });
      prevContext = translatedItems.slice(-3);
