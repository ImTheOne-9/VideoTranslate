...
function formatSubtitleFile(filePath, maxLines = 0, maxCharsPerLine = 22) {
  ...
  // Định dạng dòng: không split segment, chỉ wrap text trong từng segment
...
function getTranslationPrompt(map, targetLangName = 'Tiếng Việt') {
...
async function translateWithOpenAICompatible(map, apiKey, model, baseUrl, providerName, targetLangName = 'Tiếng Việt') {
  const prompt = getTranslationPrompt(map, targetLangName);
  const headers = {
...
async function translateWithGemini(map, apiKey, modelNameInput, targetLangName = 'Tiếng Việt') {
  const prompt = getTranslationPrompt(map, targetLangName);
...
async function translateWithOpenRouter(map, apiKey, model, targetLangName = 'Tiếng Việt') {
  const selectedModel = model || 'openrouter/owl-alpha';
  return translateWithOpenAICompatible(map, apiKey, selectedModel, 'https://openrouter.ai/api/v1/chat/completions', 'OpenRouter', targetLangName);
}

async function translateWithNineRouter(map, apiKey, model, baseUrl, targetLangName = 'Tiếng Việt') {
  const selectedModel = model || '';
  const endpoint = `${baseUrl || 'http://localhost:20128/v1'}/chat/completions`;
  return translateWithOpenAICompatible(map, apiKey, selectedModel, endpoint, '9Router', targetLangName);
}
  ...
  if ((aiProvider === 'gemini' && geminiApiKeyVal && geminiApiKeyVal.trim() !== '') ||
    ...
    try {
      ...
      for (let i = 0; i < srtArray.length; i += batchSize) {
        ...
        let translatedMap = {};
        if (isGemini) {
          translatedMap = await translateWithGemini(batchMap, apiKey, geminiModelVal, targetLangName);
        } else if (isOpenRouter) {
          translatedMap = await translateWithOpenRouter(batchMap, apiKey, openRouterModelVal, targetLangName);
        } else {
          translatedMap = await translateWithNineRouter(batchMap, ninerouterApiKeyVal, ninerouterModelVal, ninerouterBaseUrlVal, targetLangName);
        }
