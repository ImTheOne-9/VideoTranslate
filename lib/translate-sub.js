...
const { requestViaGeminiWeb } = require('./gemini-web-adapter');
...
async function requestGeminiWebJson(prompt) {
  const rawText = await requestViaGeminiWeb(prompt, { show: false });
...
async function translateSubtitles(inputPath, outputPath, geminiApiKey, maxLines = 0, maxCharsPerLine = 22, isCancelled = null) {
  ...
  // 3.5. Gemini Web dùng cùng pipeline JSON + phân tích toàn bộ SRT với Gemini API.
  if (aiProvider === 'gemini-web') {
    const providerName = 'Gemini Web';
    console.log('Đang gọi Gemini Web để phân tích toàn bộ SRT và dịch theo từng lô JSON...');
    try {
      ...
      const primary = await translateJsonBatchesWithCheckpoint({
        ...
        globalContext,
        translateBatch: (map, prevContext, sharedContext) => requestGeminiWebJson(
          getTranslationPrompt(map, targetLangName, prevContext, sharedContext)
        )
      });
