...
function validateTranslationMap(batchMap, translatedMap, options = {}) {
  ...
  for (const [id, sourceText] of Object.entries(batchMap || {})) {
    ...
    if (result.valid) valid[id] = String(candidate).trim();
...
function createTranslationCheckpoint(outputPath, context) {
  ...
  function get(item, validationOptions) {
    ...
    return validateTranslationCandidate(entry.source, entry.translated, validationOptions).valid
      ? entry.translated
      : null;
  }
  ...
  return { checkpoint, checkpointPath, failure, get, report, save, success };
}
...
function formatSubtitleFile(filePath, maxLines = 0, maxCharsPerLine = 22) {
  ...
  let srtArray = parser.fromSrt(content).map((item) => ({
    ...item,
    text: cleanWhisperParentheticalSegmentation(item.text)
  }));
...
async function translateJsonBatchesWithCheckpoint(options) {
  ...
  for (let i = 0; i < srtArray.length; i += batchSize) {
    ...
    for (const item of pending) {
      ...
      batchMap[String(item.id)] = prepared;
    }
    ...
    try {
      translatedMap = await translateBatchWithRetry(
        batchMap,
        (map) => translateBatch(map, prevContext),
        0
      );
    ...
    for (const item of pending) {
      ...
      if (batchError || !Object.hasOwn(translatedMap, id)) {
        ...
        checkpoint.failure(item, providerName, reason, batchError?.message || 'Provider không trả về ID');
        failedItems.push({ item, source, reason, error: batchError });
        continue;
      }
      const processed = postProcessText(translatedMap[id]);
      const validation = validateTranslationCandidate(source, processed, { targetLang, srcLang });
      if (!validation.valid) {
        checkpoint.failure(item, providerName, validation.reason, validation.reason);
        failedItems.push({ item, source, reason: validation.reason, error: null });
        continue;
      }
      item.text = processed;
      checkpoint.success({ id: item.id, text: source }, processed, providerName);
      translatedContext.push({ id: item.id, original: batchMap[id], translated: processed });
    }
    checkpoint.save();
    prevContext = translatedContext.slice(-12);
    console.log(
      `[Dịch] Đã xử lý ${Math.min(i + batchSize, srtArray.length)}/${srtArray.length} câu`
      + (failedItems.length ? `, đang có ${failedItems.length} câu cần fallback.` : '.')
    );
  }
...
async function fallbackFailedItemsWithNllb(options) {
  ...
  try {
    ...
    for (let index = 0; index < failedItems.length; index += 1) {
      ...
      const candidate = translated[index]?.text || '';
      ...
      const processed = postProcessText(candidate);
      failed.item.text = processed;
      checkpoint.success(
        { id: failed.item.id, text: failed.source },
        processed,
        'nllb-fallback'
...
function getTranslationPrompt(map, targetLangName = 'Tiếng Việt', prevContext = []) {
...
4. Giữ văn phong phù hợp với video ngắn trên mạng xã hội: rõ ràng, sinh động, ngắn gọn, cô đọng, súc tích, có nhịp nói tự nhiên. Ưu tiên diễn đạt ngắn gọn, lược bỏ các từ đệm không cần thiết để câu thoại không bị quá dài so với thời lượng phụ đề, nhưng vẫn đảm bảo đúng ý chính.
...
8. Giữ nguyên cấu trúc JSON gốc:
   ...
   - Giữ đúng cấu trúc từng ID JSON. Khi một câu bị chia làm nhiều khối liên tiếp, hãy phân bổ nội dung dịch tiếng Việt tương ứng vào từng khối sao cho khi đọc nối tiếp vẫn tự nhiên, đúng ngữ pháp.
...
9. Bổ sung dấu câu chuẩn cho TTS:
   - Bổ sung dấu chấm (.), dấu hỏi (?) hoặc dấu cảm (!) CHỈ ở cuối các khối phụ đề THỰC SỰ KẾT THÚC CÂU.
   - Nếu khối phụ đề đang ngắt dở giữa câu (chưa kết thúc ý), KHÔNG thêm dấu chấm ở cuối khối đó để tránh làm bộ đọc thoại TTS (OmniVoice) bị ngắt ngập ngừng giữa chừng.
...
async function translateWithOpenAICompatible(map, apiKey, model, baseUrl, providerName, targetLangName = 'Tiếng Việt', prevContext = []) {
  const prompt = getTranslationPrompt(map, targetLangName, prevContext);
  const headers = {
...
async function translateWithGemini(map, apiKey, modelNameInput, targetLangName = 'Tiếng Việt', prevContext = []) {
  const prompt = getTranslationPrompt(map, targetLangName, prevContext);
...
async function translateWithOpenRouter(map, apiKey, model, targetLangName = 'Tiếng Việt', prevContext = []) {
  const selectedModel = model || 'openrouter/owl-alpha';
  return translateWithOpenAICompatible(map, apiKey, selectedModel, 'https://openrouter.ai/api/v1/chat/completions', 'OpenRouter', targetLangName, prevContext);
}

async function translateWithOpenAI(map, apiKey, model, targetLangName = 'Tiếng Việt', prevContext = []) {
  const selectedModel = model || 'gpt-4o-mini';
  return translateWithOpenAICompatible(map, apiKey, selectedModel, 'https://api.openai.com/v1/chat/completions', 'ChatGPT (OpenAI)', targetLangName, prevContext);
}

async function translateWithNineRouter(map, apiKey, model, baseUrl, targetLangName = 'Tiếng Việt', prevContext = []) {
  const selectedModel = model || '';
  const endpoint = `${baseUrl || 'http://localhost:20128/v1'}/chat/completions`;
  return translateWithOpenAICompatible(map, apiKey, selectedModel, endpoint, '9Router', targetLangName, prevContext);
}
...
async function translateWithOpenCode(map, model = 'DeepSeek V4 Flash (Free)', targetLangName = 'Tiếng Việt', prevContext = []) {
  const prompt = getTranslationPrompt(map, targetLangName, prevContext);
  ...
  if (aiProvider === 'opencode') {
    ...
    try {
      const primary = await translateJsonBatchesWithCheckpoint({
        ...
        translateBatch: (map, prevContext) => translateWithOpenCode(
          map,
  ...
  if ((aiProvider === 'gemini' && geminiApiKeyVal && geminiApiKeyVal.trim() !== '') ||
    ...
    try {
      const primary = await translateJsonBatchesWithCheckpoint({
        ...
        isCancelled,
        batchSize: Math.min(80, srtArray.length),
        translateBatch: (map, prevContext) => {
          if (isOpenAI) {
            return translateWithOpenAI(
              map, apiKey, openaiModelVal, targetLangName, prevContext
            );
          }
          if (isGemini) {
            return translateWithGemini(
              map, apiKey, geminiModelVal, targetLangName, prevContext
            );
          }
          if (isOpenRouter) {
            return translateWithOpenRouter(
              map, apiKey, openRouterModelVal, targetLangName, prevContext
            );
          }
