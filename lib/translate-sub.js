...
function createTranslationCheckpoint(outputPath, context) {
  ...
  const signature = createCheckpointSignature({
    // Version 5 adds source-aware prompts and the API -> Web -> NLLB rescue chain.
    version: 5,
    sourceText: context.sourceText,
    targetLang: context.targetLang,
    pipeline: context.pipeline || 'shared-json-v4'
  });
...
// ViralCrawl parity: OCR can read the same hard-sub line differently across
// adjacent frames, so source-side deduplication may not catch it. Gemini can
// then normalize those variants into the exact same translated sentence. In
// that case, keep the first cue and extend it through the duplicate cues before
// TTS sees the subtitle track.
function deduplicateConsecutiveTranslatedCues(cues, options = {}) {
  if (!Array.isArray(cues) || cues.length < 2) {
    return { cues: Array.isArray(cues) ? cues : [], mergedCount: 0 };
  }

  const configuredGapMs = Number(options.maxGapMs);
  const maxGapMs = Number.isFinite(configuredGapMs) && configuredGapMs >= 0
    ? configuredGapMs
    : 2000;
  const output = [{ ...cues[0] }];
  let mergedCount = 0;
  ...
  for (const cue of cues.slice(1)) {
    ...
    const currentText = cleanSpaces(current.text);
    const previousText = cleanSpaces(previous.text);
    const currentStartMs = srtTimeToMs(current.startTime);
    const previousEndMs = srtTimeToMs(previous.endTime);
    const gapMs = currentStartMs - previousEndMs;
  ...
  output.forEach((cue, index) => {
    cue.id = String(index + 1);
  });
  return { cues: output, mergedCount };
}

function deduplicateTranslatedSubtitleFile(filePath, options = {}) {
  if (!filePath || !fs.existsSync(filePath)) return { cueCount: 0, mergedCount: 0 };
  const parser = new Parser();
  const cues = parser.fromSrt(fs.readFileSync(filePath, 'utf8'));
  const result = deduplicateConsecutiveTranslatedCues(cues, options);
  if (result.mergedCount > 0) {
    fs.writeFileSync(filePath, parser.toSrt(result.cues), 'utf8');
  }
  return { cueCount: result.cues.length, mergedCount: result.mergedCount };
}
...
async function translateJsonBatchesWithCheckpoint(options) {
  ...
  const prepareBatchMap = (items) => Object.fromEntries(items.map((item) => {
    ...
    const preparedItem = { text: prepared, durationSec };
    if (dubbingEnabled && numericFit) {
      const wps = Number.isFinite(Number(voiceWordsPerSecond)) && Number(voiceWordsPerSecond) > 0
        ? Number(voiceWordsPerSecond)
        : nhipTuDich(targetLang);
      preparedItem.maxWords = tranTuDich(
        durationSec,
        prepared,
        wps,
        targetLang === 'vi' ? Number(process.env.DUB_TILE_AM || 0.75) || 0.75 : null,
        targetLang
      );
    }
    return [String(item.id), preparedItem];
  }));
...
function getTranslationPrompt(
  ...
  const sourceKind = normalizeSubtitleSourceKind(options.sourceKind);
  const dubbingEnabled = options.dubbingEnabled === true;
  const numericFit = dubbingEnabled && options.numericFit === true;
  const sourceRecoveryRule = sourceKind === 'asr'
    ? `NGUỒN LÀ NHẬN DẠNG GIỌNG NÓI (Whisper/CapCut ASR): lỗi thường là từ ĐỒNG ÂM có thật và đúng ngữ pháp nhưng sai mạch truyện. Kiểm tra từ đáng ngờ với các dòng trước/sau; nếu lạc ý thì suy ra từ cùng âm phù hợp rồi dịch nghĩa đúng. Ví dụ: 力气 có thể bị nghe thành 利器, 要房 thành 药房, 贫道 thành 频道. Chỉ sửa từ nghe nhầm, không bịa tình tiết.`
    : (sourceKind === 'hybrid'
        ? `NGUỒN KẾT HỢP OCR VÀ ASR: vừa có thể sai mặt chữ/vô nghĩa do OCR, vừa có thể nghe nhầm từ đồng âm vẫn đúng ngữ pháp do Whisper/CapCut. Luôn đối chiếu các dòng trước/sau, sửa đúng loại lỗi rồi mới dịch; không bịa tình tiết.`
        : (sourceKind === 'user-subtitle'
            ? `NGUỒN LÀ PHỤ ĐỀ DO NGƯỜI DÙNG CUNG CẤP: tôn trọng nguyên văn; chỉ sửa lỗi rõ ràng khi ngữ cảnh chứng minh chắc chắn.`
            : `NGUỒN LÀ OCR: có thể sai mặt chữ, sinh từ vô nghĩa hoặc phá ngữ pháp. Khi gặp từ không hợp câu, dựa vào các dòng trước/sau để đoán chữ đúng rồi dịch theo ý đúng.`));
  const timingRule = !dubbingEnabled
    ? `4. VIDEO KHÔNG YÊU CẦU LỒNG TIẾNG: ưu tiên đúng nghĩa, tự nhiên và dễ đọc. Không ép ngắn hoặc cắt ý chỉ để vừa thời lượng cue.`
    : (numericFit
        ? `4. KHỚP LỒNG TIẾNG: mỗi khối có durationSec và có thể có maxWords. Viết lời nói ngắn, tự nhiên ngay từ đầu; khi có maxWords hãy cố giữ trong giới hạn đó, nhưng thà vượt nhẹ còn hơn mất ý, gộp hoặc bỏ cue.`
        : `4. KHỚP LỒNG TIẾNG: mỗi khối có durationSec để hiểu độ dài khe đọc. Viết ngắn, tự nhiên ngay từ đầu bằng cách bỏ từ đệm/từ nối/đại từ đã rõ, nhưng KHÔNG dùng công thức số từ cứng, không cắt ý và không gộp/bỏ cue.`);
  const vietnameseProperNameRule = targetLangName === 'Tiếng Việt'
   ...
   - Thương hiệu, sản phẩm và công ty hiện đại có tên Latin chính thức phải giữ dạng Latin phổ biến: 恰恰 → ChaCha; 华为 → Huawei; 小米 → Xiaomi; 抖音 → Douyin; 比亚迪 → BYD.
   - Phân biệt rõ TÊN RIÊNG với TỪ THƯỜNG: tên riêng được chuyển tên; từ thường và hư từ phải dịch theo nghĩa tiếng Việt tự nhiên, tuyệt đối không phiên âm Hán-Việt máy móc cả câu.\n`
    : '';
  const vietnameseSafetyRules = targetLangName === 'Tiếng Việt'
    ? `7b. CON SỐ VÀ ĐƠN VỊ:
   - Giữ đúng con số, phần trăm, đơn vị đo và số tiền của CHÍNH cue nguồn; không mượn số ở cue trước/sau, không tự làm tròn hoặc đổi bậc.
   - Khi dịch Trung → Việt: 万 = 10 nghìn; 十万 = 100 nghìn; 百万 = 1 triệu; 千万 = 10 triệu; 亿 = 100 triệu. \`1万\` không được dịch thành \`10 triệu\`; \`10%\` phải giữ đúng \`10%\`.

7c. KHẨU HIỆU, THÀNH NGỮ VÀ CỤM PHỦ ĐỊNH:
   - Dịch theo nghĩa của cả cụm, không ghép nghĩa từng chữ. \`不满意不要钱\`, \`不…不收费\`, \`不…包退/包换\` mang nghĩa không đạt điều kiện thì miễn phí/không thu tiền/hoàn hoặc đổi trả; \`不要钱\` không được lật thành “không miễn phí”.
   - Nguồn ASR/OCR có thể không có dấu hoặc khoảng trắng; tách cụm theo ngữ pháp trước khi dịch.

7d. CHẤM CÂU THEO MẠCH nhưng vẫn giữ 1:1: cue còn tiếp ý sang cue sau kết bằng dấu phẩy hoặc không có dấu cuối; chỉ dùng dấu chấm, dấu cảm hoặc dấu hỏi khi ý thật sự kết thúc. Không đặt dấu chấm máy móc ở mọi cue.

7e. TUYỆT ĐỐI không để sót ký tự chữ Hán lẻ trong câu dịch và không chèn ghi chú, mũi tên hoặc bình luận của người dịch.
...
${vietnameseSafetyRules}8. CẤU TRÚC JSON ĐẦU RA MẪU (FEW-SHOT EXAMPLE):
...
async function translateWithOpenAICompatible(map, apiKey, model, baseUrl, providerName, targetLangName = 'Tiếng Việt', prevContext = [], globalContext = null, translationOptions = {}) {
  const prompt = getTranslationPrompt(map, targetLangName, prevContext, globalContext, translationOptions);
...
async function translateWithGemini(map, apiKey, modelNameInput, targetLangName = 'Tiếng Việt', prevContext = [], globalContext = null, translationOptions = {}) {
  const prompt = getTranslationPrompt(map, targetLangName, prevContext, globalContext, translationOptions);
  return requestGeminiJson(prompt, apiKey, modelNameInput);
}

async function translateWithOpenRouter(map, apiKey, model, targetLangName = 'Tiếng Việt', prevContext = [], globalContext = null, translationOptions = {}) {
  const selectedModel = model || 'openrouter/owl-alpha';
  return translateWithOpenAICompatible(map, apiKey, selectedModel, 'https://openrouter.ai/api/v1/chat/completions', 'OpenRouter', targetLangName, prevContext, globalContext, translationOptions);
}

async function fallbackFailedItemsWithGeminiWeb(options) {
  const {
    failedItems,
    checkpoint,
    srcLang,
    ...
    styleRule,
    mode = 'translate',
    translateWeb = translateViaGeminiWeb,
    closeWeb = closeGeminiWebSession
  } = options;
  if (!Array.isArray(failedItems) || failedItems.length === 0) return [];
  if (isCancelled?.()) throw new Error('Đã hủy kết xuất trước khi dịch dự phòng bằng Gemini Web');

  const items = failedItems.map((failed) => failed.item);
  const sourceById = Object.fromEntries(
    failedItems.map((failed) => [String(failed.item.id), failed.source])
  );
  const stillPending = (candidates) => (Array.isArray(candidates) ? candidates : failedItems)
    .filter((failed) => !checkpoint?.get?.(
      { id: failed.item.id, text: failed.source },
      { targetLang, srcLang }
    ));
  console.log(`[Dịch dự phòng] Gemini Web đang cứu ${items.length} cue API còn thiếu/lỗi...`);
  try {
    const result = await translateWeb(items, {
      sourceById,
      checkpoint,
      outputPath: null,
      srcLang,
    ...
    const remaining = stillPending(result?.failedItems);
    console.log(`[Dịch dự phòng] Gemini Web cứu được ${items.length - remaining.length}/${items.length} cue.`);
    return remaining;
  } catch (error) {
    if (/hủy|cancel/i.test(String(error?.message || ''))) throw error;
    const remaining = stillPending(failedItems);
    console.warn(`[Dịch dự phòng] Gemini Web không dùng được (${error.message}); chuyển ${remaining.length} cue còn lại sang NLLB.`);
    return remaining.map((failed) => ({ ...failed, webFallbackError: error.message }));
  } finally {
...
async function translateWithOpenAI(map, apiKey, model, targetLangName = 'Tiếng Việt', prevContext = [], globalContext = null, translationOptions = {}) {
  const selectedModel = model || 'gpt-4o-mini';
  return translateWithOpenAICompatible(map, apiKey, selectedModel, 'https://api.openai.com/v1/chat/completions', 'ChatGPT (OpenAI)', targetLangName, prevContext, globalContext, translationOptions);
}

async function translateWithNineRouter(map, apiKey, model, baseUrl, targetLangName = 'Tiếng Việt', prevContext = [], globalContext = null, translationOptions = {}) {
  const selectedModel = model || '';
  const endpoint = `${baseUrl || 'http://localhost:20128/v1'}/chat/completions`;
  return translateWithOpenAICompatible(map, apiKey, selectedModel, endpoint, '9Router', targetLangName, prevContext, globalContext, translationOptions);
}
...
async function translateWithOpenCode(map, model = 'DeepSeek V4 Flash (Free)', targetLangName = 'Tiếng Việt', prevContext = [], globalContext = null, translationOptions = {}) {
  const prompt = getTranslationPrompt(map, targetLangName, prevContext, globalContext, translationOptions);
...
async function translateSubtitlesRaw(inputPath, outputPath, geminiApiKey, maxLines = 0, maxCharsPerLine = 22, isCancelled = null) {
  ...
  let voiceSpeed = 1;
  let voiceWordsPerSecond = null;
  let subtitleSourceKind = 'ocr';
  ...
  if (typeof geminiApiKey === 'object' && geminiApiKey !== null) {
    ...
    onTranslationBatch = typeof geminiApiKey.onTranslationBatch === 'function'
      ...
      : null;
    dubbingEnabled = geminiApiKey.dubbingEnabled === true;
    voiceSpeed = Number(geminiApiKey.voiceSpeed) || 1;
    voiceWordsPerSecond = Number.isFinite(Number(geminiApiKey.voiceWordsPerSecond))
      ? Number(geminiApiKey.voiceWordsPerSecond)
      : null;
    subtitleSourceKind = normalizeSubtitleSourceKind(geminiApiKey.subtitleSourceKind);
  ...
  // Always load the glossary because an API provider can hand failed cues to
  // Gemini Web without restarting the render.
  const translationMemoryContent = aiProvider === 'nllb' ? '' : docTm();
  const geminiBatchSize = Math.max(20, Number(process.env.GEMINI_CHUNK || 300) || 300);
  const geminiMaxChars = Math.max(1000, Number(process.env.GEMINI_MAX_CHARS || geminiBatchSize * 45) || geminiBatchSize * 45);
  const fitOverride = String(process.env.DUB_FIT_EP || '').trim();
  const numericFit = geminiWebMode !== 'spellcheck'
    && dubbingEnabled
    && (fitOverride === '1' || (fitOverride !== '0' && targetLang !== 'vi'));
  const translationPromptOptions = {
    sourceKind: subtitleSourceKind,
    dubbingEnabled,
    numericFit
  };
  const translationPolicySignature = createCheckpointSignature({
    version: 6,
    idFormat: 'ID_###',
    translationMemoryContent,
    translationStyleRule,
    geminiWebMode,
    dubbingEnabled,
    numericFit,
    voiceSpeed,
    ...
    geminiBatchSize,
    geminiMaxChars,
    retryContextRadius: Number(process.env.GEM_SOT_NGUCANH || 2) || 0
  });
  ...
  const translationCheckpoint = createTranslationCheckpoint(outputPath, {
    ...
    pipeline: aiProvider === 'gemini-web'
      ? `gemini-web-v6:source-aware:${translationPolicySignature}`
      : `shared-json-v4:source-aware:${translationPolicySignature}`
  });
  const collectPendingApiItems = () => srtArray
    .filter((item) => !translationCheckpoint.get(
      { id: item.id, text: sourceById[String(item.id)] },
      { targetLang, srcLang }
    ))
    .map((item) => ({
      item,
      source: sourceById[String(item.id)],
      reason: 'api_incomplete'
    }));
  const rescueApiFailures = async (failedItems) => {
    let remaining = await fallbackFailedItemsWithGeminiWeb({
      failedItems,
      ...
      voiceSpeed,
      voiceWordsPerSecond,
      tmContent: translationMemoryContent,
      styleRule: translationStyleRule,
      mode: geminiWebMode
    });
    remaining = await fallbackFailedItemsWithNllb({
  ...
  // 3.5. Gemini Web dùng mã [ID_xxx] + thời lượng cue, ghi SRT lũy tiến,
  // xin lại đúng ID sót rồi mới chia nhỏ phần còn lỗi bằng chính Gemini Web.
  ...
  if (aiProvider === 'opencode') {
    ...
    try {
      ...
      const remaining = await rescueApiFailures(primary.failedItems);
      const report = writeTranslatedSubtitleResult({
        parser,
        srtArray,
        outputPath,
      ...
      console.error(`${providerName} dịch chưa hoàn tất (${err.message}). Giữ cue đã tốt và chuyển phần còn lại qua Gemini Web → NLLB.`);
      const remaining = await rescueApiFailures(collectPendingApiItems());
      const report = writeTranslatedSubtitleResult({
        parser,
        srtArray,
        outputPath,
      ...
      if (remaining.length > 0) {
        const incomplete = createTranslationIncompleteError(report.translation, err);
        incomplete.translationReport = report;
        throw incomplete;
      }
      return { outputPath, report };
    }
...
async function translateSubtitles(inputPath, outputPath, geminiApiKey, maxLines = 0, maxCharsPerLine = 22, isCancelled = null) {
  const result = await translateSubtitlesRaw(
    inputPath,
    outputPath,
    geminiApiKey,
    maxLines,
  ...
  const maxGapMs = Number.isFinite(configuredGapSeconds) && configuredGapSeconds >= 0
    ...
    : 2000;
  const deduplication = deduplicateTranslatedSubtitleFile(outputPath, { maxGapMs });
  if (deduplication.mergedCount > 0) {
    console.log(
      `[Dịch phụ đề] Gộp ${deduplication.mergedCount} cue dịch TRÙNG liên tiếp `
      + `(OCR đọc lại một dòng khác chữ) → ${deduplication.cueCount} câu.`
    );
  }
  if (result?.report) {
    result.report.postTranslationDeduplication = deduplication;
  }
  return result;
}
