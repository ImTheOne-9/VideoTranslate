...
function createTranslationCheckpoint(outputPath, context) {
  ...
  const signature = createCheckpointSignature({
    ...
    translationProfile: normalizeTranslationProfile(context.translationProfile)
  });
...
async function translateJsonBatchesWithCheckpoint(options) {
  ...
  for (let i = 0; i < srtArray.length; i += batchSize) {
    ...
    for (const item of pending) {
      let prepared = prepareTextForTranslation(sourceById[String(item.id)], translationProfile);
    ...
    for (const item of pending) {
      ...
      const processed = postProcessText(translatedMap[id], translationProfile);
...
async function fallbackFailedItemsWithNllb(options) {
  ...
  try {
    ...
    for (let index = 0; index < failedItems.length; index += 1) {
      ...
      const processed = postProcessText(candidate, translationProfile);
...
function writeTranslatedSubtitleResult(options) {
  ...
  const quality = createTranslationQualityReport(
    inputContent,
    fs.readFileSync(outputPath, 'utf8'),
    translationProfile
  );
  return attachTranslationStats(quality, checkpoint.report(srtArray.length));
}
...
async function translateWithOpenAI(map, apiKey, model, targetLangName = 'Tiếng Việt', prevContext = [], translationProfile = {}) {
  const selectedModel = model || 'gpt-4o-mini';
  return translateWithOpenAICompatible(map, apiKey, selectedModel, 'https://api.openai.com/v1/chat/completions', 'ChatGPT (OpenAI)', targetLangName, prevContext, translationProfile);
}
...
async function translateSubtitles(inputPath, outputPath, geminiApiKey, maxLines = 0, maxCharsPerLine = 22, isCancelled = null) {
  ...
  if (aiProvider === 'nllb') {
    const quality = await translateWithNllb(inputPath, outputPath, srcLang, maxLines, maxCharsPerLine, isCancelled, nllbTargetLang, translationProfile);
        ...
        translateBatch: (map, prevContext) => {
          if (isOpenAI) {
            return translateWithOpenAI(
              map, apiKey, openaiModelVal, targetLangName, prevContext, translationProfile
            );
          }
          if (isGemini) {
            return translateWithGemini(
              map, apiKey, geminiModelVal, targetLangName, prevContext, translationProfile
            );
          }
          if (isOpenRouter) {
            return translateWithOpenRouter(
              map, apiKey, openRouterModelVal, targetLangName, prevContext, translationProfile
            );
          }
  ...
  for (let i = 0; i < googlePending.length; i += batchSize) {
    ...
    const textToTranslate = batch.map(item => {
      const clean = prepareTextForTranslation(sourceById[String(item.id)], translationProfile);
    ...
    try {
      ...
      if (translatedParts.length === batch.length) {
        ...
        for (const item of batch) {
          ...
          try {
            const source = sourceById[String(item.id)];
            const cleanText = prepareTextForTranslation(source, translationProfile);
