...
_RE_TAP = re.compile(
    r"(?P<tag>t[aậ]p|ph[aầ]n|ep(?:isode)?|p)\s*[.\-]?\s*(?P<so>\d+)",
    re.IGNORECASE,
)
...
def _media_path_parts(aweme_item: Dict) -> Tuple[str, str]:
    ...
    if ctype == "search":
        ...
        folder = f"{safe_nick}_{_sec[-8:]}" if _sec else safe_nick
...
class DouYinCrawler(AbstractCrawler):
    ...
    async def get_chase_series(self) -> None:
        ...
        for video_url in config.DY_SPECIFIED_ID_LIST:
            ...
            ten_goc, so_tap = tach_ten_goc_tap(title)
            if not ten_goc:
                utils.logger.error(f"[get_chase_series] Tiêu đề '{title[:50]}' KHÔNG có số tập "
                                   f"(Tập/Phần/P.N/EP/#N) — không xác định được đây là video thuộc bộ nào.")
                continue
            sec_uid = (aweme_detail.get("author") or {}).get("sec_uid", "")
            if not sec_uid:
                utils.logger.error(f"[get_chase_series] Video {video_info.aweme_id} thiếu sec_uid tác giả — bỏ qua.")
                continue
            utils.logger.info(f"[get_chase_series] '{title[:50]}' → tên gốc='{ten_goc}' (video này là Tập {so_tap}) "
                              f"— quét kênh tác giả tìm mọi tập cùng bộ...")

            _os_env_bak = os.environ.get("DY_CHASE_SERIES"), os.environ.get("DY_CHASE_TITLE")
            os.environ["DY_CHASE_SERIES"] = "1"
            os.environ["DY_CHASE_TITLE"] = ten_goc
            try:
                creator_info: Dict = await self.dy_client.get_user_info(sec_uid)
                if creator_info:
                    await douyin_store.save_creator(sec_uid, creator=creator_info)
                selected = await self._collect_user_posts(sec_uid, "newest", 10 ** 9)   # limit vô nghĩa ở nhánh CHASE (dừng theo Tập 1)
                selected = selected[:max(1, min(5000, config.CRAWLER_MAX_NOTES_COUNT))]
                utils.logger.info(f"[get_chase_series] '{ten_goc}': tìm được {len(selected)} tập — đang tải...")
                if os.environ.get("MC_GET_MEDIAS", "1") == "0":
                    for _aw in selected:
                        try:
                            await douyin_store.update_douyin_aweme(aweme_item=_aw)
                        except Exception as _e:
                            utils.logger.warning(f"[get_chase_series] preview ghi list item lỗi: {_e}")
                else:
                    await self.fetch_creator_video_detail(selected)
                video_ids = [v.get("aweme_id") for v in selected]
                await self.batch_get_note_comments(video_ids)
            finally:
                for _k, _v in zip(("DY_CHASE_SERIES", "DY_CHASE_TITLE"), _os_env_bak):
                    if _v is None:
