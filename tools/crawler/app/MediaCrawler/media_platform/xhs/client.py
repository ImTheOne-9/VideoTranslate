...
from tenacity import retry, stop_after_attempt, stop_after_delay, wait_fixed, retry_if_not_exception_type
...
class XiaoHongShuClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    # 🐛 fix (audit cải tiến #G): thêm trần THỜI GIAN (stop_after_delay) bên cạnh trần SỐ LẦN cũ — hàm này bị
    # get_note_by_id_from_html (dòng ~677, cũng 3 lần) gọi LỒNG bên trong → xấu nhất 3×3=9 lần gọi mạng, mỗi
    # lần chờ tới `timeout` (mặc định 60s, env MC_XHS_TIMEOUT) + wait_fixed(1) → có thể treo ~9 PHÚT không có
    # trần nào chặn (đúng triệu chứng đã ghi ở web_app._watchdog: "Xiaohongshu hay treo... retry lồng... user
    # nhìn 0% hàng chục phút tưởng đơ"). 45s ≈ dưới 1× timeout mặc định → xấu nhất chặn sau ĐÚNG 1 lần thử (còn
    # hơn 3), không đổi hành vi ca thường (thử 1-2 lần đã xong trong vài giây).
    @retry(stop=(stop_after_attempt(3) | stop_after_delay(45)), wait=wait_fixed(1),
    ...
    async def get_note_media(self, url: str) -> Union[bytes, None]:
        ...
        async with make_async_client(proxy=self.proxy) as client:
            try:
                ...
                if not data:
                    utils.logger.error(
                        f"[XiaoHongShuClient.get_note_media] HTTP 200 nhưng body rỗng — {url}"
                    )
                    return None
                content_type = (response.headers.get("content-type") or "").lower()
                if content_type.startswith(("text/", "application/json")):
                    utils.logger.error(
                        f"[XiaoHongShuClient.get_note_media] HTTP 200 nhưng content-type '{content_type}' không phải media — {url}"
                    )
                    return None
                is_video = "video" in content_type or url.split("?", 1)[0].lower().endswith((".mp4", ".m4v", ".mov"))
                if is_video:
                    valid_mp4 = len(data) >= 12 and any(
                        data[offset:offset + 4] in (b"ftyp", b"moov", b"free", b"skip", b"wide")
                        for offset in (4, 8)
                    )
                    if not valid_mp4:
                        utils.logger.error(
                            f"[XiaoHongShuClient.get_note_media] tải {len(data)} bytes nhưng thiếu chữ ký MP4 — bỏ file lỗi"
                        )
                        return None
                return data
            except (
    ...
    async def pong(self) -> bool:
        ...
        ping_flag = False
        self.last_pong_code = None   # ViralCrawl: lưu code lý do pong fail để core phân biệt -104 风控 vs chưa-login
        self.last_pong_msg = ""
        try:
            ...
            if self_info and self_info.get("data", {}).get("result", {}).get("success"):
                ...
                self.last_pong_code = self_info.get("code")
                self.last_pong_msg = self_info.get("msg") or ""
                utils.logger.warning(
                    f"[XiaoHongShuClient.pong] not logged in: code={self.last_pong_code} msg={self.last_pong_msg}")
    ...
    # 🐛 fix (audit cải tiến #G): trần thời gian cho lớp NGOÀI — mỗi lần thử ở đây gọi self.request() (dòng
    # ~115, tự có trần 45s riêng) nên xấu nhất/lần ~45-60s; 100s ≈ 2 lần thử NGOÀI là dừng, thay vì 3 lần ×
    # (tới 9 lần request thật bên trong) ~9 phút như trước. Xem chi tiết ở comment request().
    @retry(stop=(stop_after_attempt(3) | stop_after_delay(100)), wait=wait_fixed(1))
