...
def ram_gb_tong() -> float:
    ...
    try:
        ...
        if sys.platform == "darwin":
            out = subprocess.run(["sysctl", "-n", "hw.memsize"], capture_output=True, text=True, timeout=3, creationflags=_NO_WIN)
...
def ram_gb_avail() -> float:
    """RAM RỖI THẬT (GB tuyệt đối, không phải %). Lỗi/không xác định -> 0.0.

    🐛 fix (audit cải tiến #A): trước đây một số file phụ và `ocr_bulk.py` mỗi file TỰ VIẾT LẠI y hệt
    struct `ctypes` đọc `ullAvailPhys` — đúng lớp trùng lặp mà module này (xem docstring đầu file) được tạo
    ra để triệt tiêu nhưng chưa triệt hết. Nay gom về đây, 2 file kia gọi lại hàm này.
    Cũng dùng để tính ngưỡng RAM% các nơi khác (render_worker tự-restart, kill-browser) theo GB TUYỆT ĐỐI
    thay vì % cố định — máy 8GB và máy 64GB cần ngưỡng % khác nhau để cùng đạt "còn X GB rỗi thật"."""
    try:
        if sys.platform == "win32":
            class _MS(ctypes.Structure):
                _fields_ = [("dwLength", ctypes.c_ulong), ("dwMemoryLoad", ctypes.c_ulong),
        ...
        if sys.platform == "darwin":
            tong = ram_gb_tong()
            pct = ram_pct_dung()
            return round(tong * (100 - pct) / 100.0, 2) if tong else 0.0
        with open("/proc/meminfo", "r") as f:      # Linux
            for line in f:
...
def ram_pct_dung() -> int:
    ...
    try:
        if sys.platform == "win32":
            ...
            return int(m.load)
        if sys.platform == "darwin":
            out = subprocess.run(["vm_stat"], capture_output=True, text=True, timeout=3, creationflags=_NO_WIN).stdout
