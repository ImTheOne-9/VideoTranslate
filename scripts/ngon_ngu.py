...
# 🐛 fix (audit BAO-CAO-BUG-2 #B22): thiếu "rednote" — code cào/tải THẬT lưu video RedNote vào thư mục
# "rednote" (xem web_app._xhs_tai_links_worker), không phải "xhs" → nếu chỉ có "xhs" ở đây,
# nguon_theo_nen_tang("rednote") trả về ngôn ngữ ĐÍCH thay vì "zh" — video RedNote (tiếng Trung) có thể bị
# đoán nhầm "cùng ngôn ngữ đích" ở bước đầu (dù có lưới an toàn OCR dò Hán trên hình phía sau).
NEN_TANG_TRUNG = ("douyin", "bili", "bilibili", "xhs", "rednote", "weibo", "wb", "kuaishou")
...
def giong_nam_nu(lang=None):
    """(giọng NAM, giọng NỮ) mặc định của ngôn ngữ — cho tính năng '1 video 2 giọng' (gán câu-nam→giọng-nam,
    câu-nữ→giọng-nữ). Đọc field 'nam'/'nu' trong LANGS. vi: ('vi-VN-NamMinhNeural', 'vi-VN-HoaiMyNeural').
    BUG THẬT (user báo 2-giọng đích Hàn/Nhật/Đức/TBN/Pháp/Ý/Bồ ra sai giọng): 7 ngôn ngữ Supertonic có
    field nam/nu = 'M1'/'F1' (TOKEN Supertonic, KHÔNG phải locale edge) → khi engine lùi edge, long_tieng_edge
    nhận 'M1'/'F1' làm locale → edge-tts fail/đọc sai. Fix TẠI NGUỒN: giọng KHÔNG phải locale edge hợp lệ
    (≥2 dấu '-') → lùi voice_mac_dinh(lang) (giọng edge thật của đích đó) cho CẢ nam lẫn nữ."""
    _lg = (lang or target_lang() or "vi").strip().lower()
    d = LANGS.get(_lg, LANGS["vi"])
    _nam = d.get("nam", "vi-VN-NamMinhNeural")
    _nu = d.get("nu", "vi-VN-HoaiMyNeural")
    _mac_dinh = voice_mac_dinh(_lg)
    if str(_nam or "").count("-") < 2:   # 'M1'/token → không phải locale edge → lùi giọng mặc định đích
        _nam = _mac_dinh
    if str(_nu or "").count("-") < 2:
        _nu = _mac_dinh
    return (_nam, _nu)


# Ngôn ngữ có giọng Piper OFFLINE chất-lượng-cao (khớp PIPER_LANG_VOICE trong localize.py — 6 đích có model
# HIGH: en/de/es/pl/uk/kk). vi dùng Piper riêng (ngochuyen/NghiTTS) → cũng offline. medium/low (id/ru/ar...)
# CỐ Ý không vào đây (chưa đủ hay → giữ edge). Sửa 1 chỗ khi thêm giọng high.
_PIPER_HIGH = {"en", "de", "es", "pl", "uk", "kk"}


def tts_mac_dinh(lang=None):
    """Engine TTS MẶC ĐỊNH theo ngôn ngữ đích — CHỈ Piper high mới offline (nhanh, không throttle, chạy song
    song với 1 edge): Piper high (vi/en/de/es/pl/uk/kk) → 'piper'; KHÔNG có high → 'edge' (online).
    Supertonic BỎ (user: chậm). User chọn engine cụ thể ở UI thì TÔN TRỌNG (hàm này chỉ dùng khi opts.tts rỗng)."""
    l = (lang or target_lang() or "").strip().lower()
    if l == "vi" or l in _PIPER_HIGH:
        return "piper"
    return "edge"
